...
def run_smt_foundational_tests(omega_values_to_test: List[int]):
    ...
    num_main_smt_tests = 11 # Original set: add(c,a,id,inv_ex,inv_uq), mul(c,a,zd), distrib, cancel(a,m)
    num_sub_tests = 4  # sub(id,stuck,noncomm,nonassoc)
    num_div_tests = 3 + 3 + 1 # div_unio_interactions (3 parts), div_dfi_cases (3 parts), div_round_trip (1 part)
                           # Note: some of these might be skipped for certain Omega, affecting "skipped" vs "passed" counts.
                           # The _report_native_test_result used for SMT skips handles this.
    total_intended_smt_tests = num_main_smt_tests + num_sub_tests + num_div_tests

    if not SMT_MODE_AVAILABLE:
        print("  SMT tests cannot run because PySMT/Solver is not available.")
        for omega_val in omega_values_to_test:
            _initialize_smt_omega_results(omega_val)
            smt_test_results_summary[omega_val]["skipped"] += total_intended_smt_tests
        return
