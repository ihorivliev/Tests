# Script Name: AVC_GeneralSubtraction_DirectSpan_Chained_Test.py
...
if __name__ == "__main__":
    print("====== AVC General Subtraction Direct Span Chained Test ======\n")

    P_A_val_sym = Symbol("PA_val_GSDSChain", solver_INT_TYPE)
    P_B_val_sym = Symbol("PB_val_GSDSChain", solver_INT_TYPE)
    
    A_L = Symbol("A_L_GSDSChain", solver_INT_TYPE)
    B_L = Symbol("B_L_GSDSChain", solver_INT_TYPE)
    C_L = Symbol("C_L_GSDSChain", solver_INT_TYPE)
    
    ZS_const_L = Symbol("ZS_const_L_GSDSChain", solver_INT_TYPE) # For negation tests
    ...
    for current_local_span_S_py in LOCAL_SPANS_TO_TEST:
        ...
        print(f"\n\n===== DIRECT SPAN SUBTRACTION CHAINED TESTS WITH S = {current_local_span_S_py} (Omega_Canonical = S) =====\n")
        ...
        # Ensure inputs are valid local values for the frame
        all_local_inputs = [A_L, B_L, C_L, ZS_const_L]
        for inp_val_sym in all_local_inputs: 
            s.add_assertion(And(inp_val_sym >= P_A_val_sym, inp_val_sym <= P_B_val_sym))
        
        s.add_assertion(Equals(ZS_const_L, P_A_val_sym)) # Define ZS_const_L as Local ZS

        # --- GSDS-8: Distributivity of Mul over Sub ---
        if current_local_span_S_py >= 2: # Needs DFIs to be meaningful
            # GSDS-8.1: C_L * (A_L - B_L) ~ (C_L*A_L) - (C_L*B_L)
            s.push()
            ...
            sub_ab_gsds81 = define_local_op_direct_span_result(smart_raw_sub_canonical_logic_builder, A_L, B_L, P_A_val_sym, P_B_val_sym, f"subAB_gsds81_S{current_local_span_S_py}")
            lhs_gsds81 = define_local_op_direct_span_result(raw_mul_logic_builder, C_L, sub_ab_gsds81["val_L_sym"], P_A_val_sym, P_B_val_sym, f"lhs_gsds81_S{current_local_span_S_py}")
            prod_ca_gsds81 = define_local_op_direct_span_result(raw_mul_logic_builder, C_L, A_L, P_A_val_sym, P_B_val_sym, f"prodCA_gsds81_S{current_local_span_S_py}")
            prod_cb_gsds81 = define_local_op_direct_span_result(raw_mul_logic_builder, C_L, B_L, P_A_val_sym, P_B_val_sym, f"prodCB_gsds81_S{current_local_span_S_py}")
            rhs_gsds81 = define_local_op_direct_span_result(smart_raw_sub_canonical_logic_builder, prod_ca_gsds81["val_L_sym"], prod_cb_gsds81["val_L_sym"], P_A_val_sym, P_B_val_sym, f"rhs_gsds81_S{current_local_span_S_py}")
            prop_gsds81 = avc_equiv_local_val(lhs_gsds81["val_L_sym"], rhs_gsds81["val_L_sym"], P_A_val_sym, P_B_val_sym)
            setup_gsds81 = (sub_ab_gsds81["defining_assertions"] + lhs_gsds81["defining_assertions"] + prod_ca_gsds81["defining_assertions"] + 
                            prod_cb_gsds81["defining_assertions"] + rhs_gsds81["defining_assertions"])
            prove_or_find_counterexample(s, f"GSDS-8.1 L-Distrib Mul over Sub (DFIs) (S={current_local_span_S_py})", setup_gsds81, prop_gsds81,
                                         op_result_dicts_for_debug=[sub_ab_gsds81, lhs_gsds81, prod_ca_gsds81, prod_cb_gsds81, rhs_gsds81])
            s.pop()

            # GSDS-8.2: (A_L - B_L) * C_L ~ (A_L*C_L) - (B_L*C_L)
            s.push()
            s.add_assertion(is_local_DFI_val(A_L, P_A_val_sym, P_B_val_sym))
            s.add_assertion(is_local_DFI_val(B_L, P_A_val_sym, P_B_val_sym))
            s.add_assertion(is_local_DFI_val(C_L, P_A_val_sym, P_B_val_sym))

            sub_ab_gsds82 = define_local_op_direct_span_result(smart_raw_sub_canonical_logic_builder, A_L, B_L, P_A_val_sym, P_B_val_sym, f"subAB_gsds82_S{current_local_span_S_py}")
            lhs_gsds82 = define_local_op_direct_span_result(raw_mul_logic_builder, sub_ab_gsds82["val_L_sym"], C_L, P_A_val_sym, P_B_val_sym, f"lhs_gsds82_S{current_local_span_S_py}")
            prod_ac_gsds82 = define_local_op_direct_span_result(raw_mul_logic_builder, A_L, C_L, P_A_val_sym, P_B_val_sym, f"prodAC_gsds82_S{current_local_span_S_py}")
            prod_bc_gsds82 = define_local_op_direct_span_result(raw_mul_logic_builder, B_L, C_L, P_A_val_sym, P_B_val_sym, f"prodBC_gsds82_S{current_local_span_S_py}")
            rhs_gsds82 = define_local_op_direct_span_result(smart_raw_sub_canonical_logic_builder, prod_ac_gsds82["val_L_sym"], prod_bc_gsds82["val_L_sym"], P_A_val_sym, P_B_val_sym, f"rhs_gsds82_S{current_local_span_S_py}")
            prop_gsds82 = avc_equiv_local_val(lhs_gsds82["val_L_sym"], rhs_gsds82["val_L_sym"], P_A_val_sym, P_B_val_sym)
            setup_gsds82 = (sub_ab_gsds82["defining_assertions"] + lhs_gsds82["defining_assertions"] + prod_ac_gsds82["defining_assertions"] + 
                            prod_bc_gsds82["defining_assertions"] + rhs_gsds82["defining_assertions"])
            prove_or_find_counterexample(s, f"GSDS-8.2 R-Distrib Mul over Sub (DFIs) (S={current_local_span_S_py})", setup_gsds82, prop_gsds82,
                                         op_result_dicts_for_debug=[sub_ab_gsds82, lhs_gsds82, prod_ac_gsds82, prod_bc_gsds82, rhs_gsds82])
            s.pop()
        else:
            print(f"--- SKIPPING GSDS-8 for S={current_local_span_S_py} (Requires DFI inputs) ---")


        # --- GSDS-9: Properties of Negation (-A_L is ZS_L - A_L) ---
        # GSDS-9.1: ZS_L - (ZS_L - A_L) ~ A_L 
        s.push()
        neg_A = define_local_op_direct_span_result(smart_raw_sub_canonical_logic_builder, ZS_const_L, A_L, P_A_val_sym, P_B_val_sym, f"negA_gsds91_S{current_local_span_S_py}")
        neg_neg_A = define_local_op_direct_span_result(smart_raw_sub_canonical_logic_builder, ZS_const_L, neg_A["val_L_sym"], P_A_val_sym, P_B_val_sym, f"negNegA_gsds91_S{current_local_span_S_py}")
        prop_gsds91 = avc_equiv_local_val(neg_neg_A["val_L_sym"], A_L, P_A_val_sym, P_B_val_sym)
        setup_gsds91 = neg_A["defining_assertions"] + neg_neg_A["defining_assertions"]
        prove_or_find_counterexample(s, f"GSDS-9.1 ZS_L-(ZS_L-A_L) ~ A_L (S={current_local_span_S_py})", setup_gsds91, prop_gsds91,
                                     op_result_dicts_for_debug=[neg_A, neg_neg_A])
        s.pop()

        # GSDS-9.2: A_L - B_L ~ A_L + (ZS_L - B_L)
        s.push()
        lhs_gsds92 = define_local_op_direct_span_result(smart_raw_sub_canonical_logic_builder, A_L, B_L, P_A_val_sym, P_B_val_sym, f"lhs_gsds92_S{current_local_span_S_py}")
        neg_B_gsds92 = define_local_op_direct_span_result(smart_raw_sub_canonical_logic_builder, ZS_const_L, B_L, P_A_val_sym, P_B_val_sym, f"negB_gsds92_S{current_local_span_S_py}")
        rhs_gsds92 = define_local_op_direct_span_result(smart_raw_add_logic_builder, A_L, neg_B_gsds92["val_L_sym"], P_A_val_sym, P_B_val_sym, f"rhs_gsds92_S{current_local_span_S_py}")
        prop_gsds92 = avc_equiv_local_val(lhs_gsds92["val_L_sym"], rhs_gsds92["val_L_sym"], P_A_val_sym, P_B_val_sym)
        setup_gsds92 = lhs_gsds92["defining_assertions"] + neg_B_gsds92["defining_assertions"] + rhs_gsds92["defining_assertions"]
        prove_or_find_counterexample(s, f"GSDS-9.2 A_L-B_L ~ A_L+(ZS_L-B_L) (S={current_local_span_S_py})", setup_gsds92, prop_gsds92,
                                     op_result_dicts_for_debug=[lhs_gsds92, neg_B_gsds92, rhs_gsds92])
        s.pop()
        
        # --- GSDS-10: Subtraction Transformation Properties ---
        # GSDS-10.1: (A_L - B_L) - C_L ~ A_L - (B_L + C_L)
        s.push()
        sub_ab_gsds101 = define_local_op_direct_span_result(smart_raw_sub_canonical_logic_builder, A_L, B_L, P_A_val_sym, P_B_val_sym, f"sAB_gsds101_S{current_local_span_S_py}")
        lhs_gsds101 = define_local_op_direct_span_result(smart_raw_sub_canonical_logic_builder, sub_ab_gsds101["val_L_sym"], C_L, P_A_val_sym, P_B_val_sym, f"lhs_gsds101_S{current_local_span_S_py}")
        add_bc_gsds101 = define_local_op_direct_span_result(smart_raw_add_logic_builder, B_L, C_L, P_A_val_sym, P_B_val_sym, f"aBC_gsds101_S{current_local_span_S_py}")
        rhs_gsds101 = define_local_op_direct_span_result(smart_raw_sub_canonical_logic_builder, A_L, add_bc_gsds101["val_L_sym"], P_A_val_sym, P_B_val_sym, f"rhs_gsds101_S{current_local_span_S_py}")
        prop_gsds101 = avc_equiv_local_val(lhs_gsds101["val_L_sym"], rhs_gsds101["val_L_sym"], P_A_val_sym, P_B_val_sym)
        setup_gsds101 = (sub_ab_gsds101["defining_assertions"] + lhs_gsds101["defining_assertions"] + 
                         add_bc_gsds101["defining_assertions"] + rhs_gsds101["defining_assertions"])
        prove_or_find_counterexample(s, f"GSDS-10.1 (A_L-B_L)-C_L ~ A_L-(B_L+C_L) (S={current_local_span_S_py})", setup_gsds101, prop_gsds101,
                                     op_result_dicts_for_debug=[sub_ab_gsds101,lhs_gsds101,add_bc_gsds101,rhs_gsds101])
        s.pop()

        # GSDS-10.2: A_L - (B_L - C_L) ~ (A_L - B_L) + C_L
        s.push()
        sub_bc_gsds102 = define_local_op_direct_span_result(smart_raw_sub_canonical_logic_builder, B_L, C_L, P_A_val_sym, P_B_val_sym, f"sBC_gsds102_S{current_local_span_S_py}")
        lhs_gsds102 = define_local_op_direct_span_result(smart_raw_sub_canonical_logic_builder, A_L, sub_bc_gsds102["val_L_sym"], P_A_val_sym, P_B_val_sym, f"lhs_gsds102_S{current_local_span_S_py}")
        sub_ab_gsds102 = define_local_op_direct_span_result(smart_raw_sub_canonical_logic_builder, A_L, B_L, P_A_val_sym, P_B_val_sym, f"sAB_gsds102_S{current_local_span_S_py}")
        rhs_gsds102 = define_local_op_direct_span_result(smart_raw_add_logic_builder, sub_ab_gsds102["val_L_sym"], C_L, P_A_val_sym, P_B_val_sym, f"rhs_gsds102_S{current_local_span_S_py}")
        prop_gsds102 = avc_equiv_local_val(lhs_gsds102["val_L_sym"], rhs_gsds102["val_L_sym"], P_A_val_sym, P_B_val_sym)
        setup_gsds102 = (sub_bc_gsds102["defining_assertions"] + lhs_gsds102["defining_assertions"] + 
                         sub_ab_gsds102["defining_assertions"] + rhs_gsds102["defining_assertions"])
        prove_or_find_counterexample(s, f"GSDS-10.2 A_L-(B_L-C_L) ~ (A_L-B_L)+C_L (S={current_local_span_S_py})", setup_gsds102, prop_gsds102,
                                     op_result_dicts_for_debug=[sub_bc_gsds102,lhs_gsds102,sub_ab_gsds102,rhs_gsds102])
        s.pop()

        del s
        print("-" * 80)

    print("\n====== AVC General Subtraction Direct Span Chained Test Complete ======")