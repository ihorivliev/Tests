...
def avc_add_smt_logic(a: Dict[str, FNode], b: Dict[str, FNode],
    ...
    """
    SMT Logic for avc_add (⊕_v1.1: DFI overflow to AREO_UNIO).
    Unio (any aspect) is universal additive identity.
    DFI + DFI: sum if < Omega_eff; else AREO_UNIO.
    """
    a_is_unio = Or(a["is_zero"], a["is_areo"])
    b_is_unio = Or(b["is_zero"], b["is_areo"])

    # Case 1: 'a' is Unio. Result 'res' should be identical to 'b'.
    # SymbolicAssign(res, b)
    res_is_b_formula = And(Iff(res["is_zero"], b["is_zero"]), 
                           Iff(res["is_areo"], b["is_areo"]), 
                           Iff(res["is_finite"], b["is_finite"]), 
                           Equals(res["val"], b["val"]))
    case1_a_is_unio = Implies(a_is_unio, res_is_b_formula)

    # Case 2: 'b' is Unio AND 'a' is DFI. Result 'res' should be identical to 'a'.
    # SymbolicAssign(res, a)
    res_is_a_formula = And(Iff(res["is_zero"], a["is_zero"]), 
                           Iff(res["is_areo"], a["is_areo"]), 
                           Iff(res["is_finite"], a["is_finite"]), 
                           Equals(res["val"], a["val"]))
    case2_b_is_unio_a_is_dfi = Implies(And(Not(a_is_unio), b_is_unio), res_is_a_formula)
    
    # Case 3: Both 'a' and 'b' are DFI.
    sum_val = a["val"] + b["val"] # Symbolic integer addition

    # Result 'res' is DFI sum:
    res_is_FP_sum_formula = And(Not(res["is_zero"]), 
                                Not(res["is_areo"]), 
                                res["is_finite"], 
                                Equals(res["val"], sum_val))
    
    # Result 'res' is AREO_UNIO (for overflow, v1.1 refinement):
    # This uses the same structure as res_is_AU_formula from your avc_sub_smt_logic
    res_is_AU_overflow_formula = And(Not(res["is_zero"]), 
                                     res["is_areo"], 
                                     Not(res["is_finite"]), 
                                     Equals(res["val"], omega_smt_node))

    case3_dfi_dfi = Implies(
        And(a["is_finite"], b["is_finite"]), # If both a and b are DFI
        Ite(
            sum_val < omega_smt_node,       # If sum < Omega
            res_is_FP_sum_formula,          # Then result is DFI sum
            res_is_AU_overflow_formula      # Else (sum >= Omega), result is AREO_UNIO
        )
    )
    
    # The overall logic must cover these disjoint cases.
    # If a is Unio, case1 applies.
    # Else (a is DFI):
    #   If b is Unio, case2 applies.
    #   Else (b is DFI), case3 applies.
    # This can be structured as nested ITEs or a conjunction of implications
    # ensuring only one operative rule for any valid a,b pair.
    # The original script used And(case1, case2, case3) which works because
    # the preconditions of the Implies are mutually exclusive for (Unio,X), (DFI,Unio), (DFI,DFI).
