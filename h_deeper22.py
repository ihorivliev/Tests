...
def run_T1_find_dual_monotonic_pos_tweaked(
    ...
    print(f"         mapped AVCA ⊕ is monotone w.r.t. le_add (with {min_extra_edges_add_min} <= non-refl. edges <= {min_extra_edges_add_max}), AND")
...
if __name__ == "__main__":
    ...
    print("    Constraining le_add to have AT MOST 1 non-reflexive edge (min_extra_edges_add_max=1).")
    print("    And le_add must have AT LEAST min_extra_edges_add_min non-reflexive edges.")
    
    print(f"\n--- Test Case: le_add (0-1 edges), le_mul (≥0 edges) ---")
