...
def run_smt_foundational_tests(omega_values_to_test: List[int]):
    ...
    num_intended_smt_tests_approx = 46 
    ...
    if smt_test_failures_details:
        ...
        for failure in sorted(smt_test_failures_details, key=lambda x: x['omega']): # type: ignore
            ...
            if model_is_available and model_str_to_print and model_str_to_print != "N/A" and not isinstance(model_str_to_print, Exception):
                 label = "Model (Witness/Counterexample)" # Default label
                 
                 # Refined labeling based on whether the outcome matched the expectation for this failure item
                 if (observed_holds_failure == expected_to_hold_failure):
                     # This block should ideally not be reached if an item is in "failures_details"
                     # as "failures_details" should only contain tests that FAILED against expectation.
                     # However, if it does, it means it passed but was logged as failure.
                     if is_existential_failure and observed_holds_failure: label = "Witness (Error: Passed, but in failures list)"
                     elif not is_existential_failure and not observed_holds_failure: label = "Counterexample (Error: Passed, but in failures list)"
                     else: label = "Model (Passed, but in failures list)"
                 else: # Actual Failure against expectation (this is the expected path for items in this list)
                     ...
                     elif not is_existential_failure and observed_holds_failure: label = "Details (Expected Counterexample Not Found / Unexpected Hold)"

                 # Condition to avoid printing redundant "No witness found" or already displayed SMT exceptions
                 condition_to_avoid_printing_model_detail = (
                     (is_existential_failure and not observed_holds_failure and "No witness found" in model_str_to_print) or
                     (not is_existential_failure and observed_holds_failure and model_str_to_print == "N/A") or
                 ...
                 if not condition_to_avoid_printing_model_detail:
