...
def smt_test_D2_resetting_subtraction_well_definedness(omega_val_py: int):
    ...
    if omega_val_py <= 1:
        ...
        print(f"      (Test D.2 N/A for Omega={omega_val_py} as DFI elements are needed or behavior is trivial)")
        return
    ...
    def hypothetical_resetting_sub_logic(a_h: Dict[str, FNode], b_h: Dict[str, FNode], res_h: Dict[str, FNode], omega_h: FNode) -> FNode:
        ...
        # Standard behavior for b_h is Unio or both DFI (taken from avc_sub_smt_logic)
        ...
        # Hypothetical: Unio - DFI = Fp(Omega - DFI_val) -- this is the "resetting" part
        res_h_is_FP_reset = And(Not(res_h["is_zero"]), Not(res_h["is_areo"]), res_h["is_finite"], 
                                Equals(res_h["val"], omega_h - b_h["val"]),
                                (omega_h - b_h["val"]) > Int(0), # Ensure result is valid DFI value
                                (omega_h - b_h["val"]) < omega_h) 
        case_unio_dfi_reset = Implies(And(a_h_is_unio, b_h["is_finite"]), res_h_is_FP_reset)
        
        return And(case_b_unio, case_dfi_dfi, case_unio_dfi_reset)
    ...
    prove_or_find_counterexample_smt(property_name, omega_val_py, setup, property_formula,
                                     [a1, a2, b1, b2, res1, res2], expect_property_to_hold=False) # Expected to fail for Omega > 1
...
def smt_test_M_zero_divisors_mul(omega_val_py: int):
    ...
    if omega_val_py == 1:
        prove_or_find_counterexample_smt(property_name, omega_val_py, [], FALSE(), [], expect_property_to_hold=False, is_existential=True)
        print(f"      (Test M N/A for Omega={omega_val_py} - no non-ZS DFI elements)")
        return
    ...
    prove_or_find_counterexample_smt(property_name, omega_val_py, setup, property_formula,
                                     [a,b,res_ab, ZU_const], expect_property_to_hold=True, is_existential=True)
...
def smt_test_N_dfi_non_closure(omega_val_py: int):
    ...
    if omega_val_py <= 1:
        ...
        smt_test_results_summary[omega_val_py]["passed"] += 4 # For 4 ops
        return
    ...
    for op_name_str, op_logic_func in ops_logic_map.items():
        ...
        prove_or_find_counterexample_smt(current_property_name, omega_val_py, setup, property_formula,
                                         [a,b,res_ab], expect_property_to_hold=True, is_existential=True)
