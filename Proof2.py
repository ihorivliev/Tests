# ==========================================================
# AVCA_Ω – SMT Proof Script (V8 - Final Scope & Call Corrections)
...
def check_smt(solver: Solver, formula: object, expected_sat: bool,
    ...
    elif expected_sat and result and print_model_on_debug: # Expected SAT and got SAT
...
def get_avca_axiom_formulas(Omega_val: int,
                            add_func_symbol_to_use: object,
                            mul_func_symbol_to_use: object) -> \
                            Tuple[object, Callable, Callable, List[object]]:
    """
    ...
    # Quantifier variables for these axioms, made unique with Omega_val and func name
    # to avoid any potential SMT symbol clashes if this function were called weirdly
    # (though with fresh solvers per Omega, it's mostly for extreme safety)
    base_q_name = add_func_symbol_to_use.symbol_name().replace(f"_O{Omega_val}","") # e.g., "avca_add_main" or "avca_add2_uniq"
    x_ax_gen = Symbol(f"x_ax_{base_q_name}_O{Omega_val}", INT)
    y_ax_gen = Symbol(f"y_ax_{base_q_name}_O{Omega_val}", INT)
    ...
    current_axioms_list = []
    # Id⊕
    current_axioms_list.append(ForAll([x_ax_gen], Implies(in_S_Omega_f(x_ax_gen), Equals(add_func_symbol_to_use(U_smt_c, x_ax_gen), x_ax_gen))))
    current_axioms_list.append(ForAll([x_ax_gen], Implies(in_S_Omega_f(x_ax_gen), Equals(add_func_symbol_to_use(x_ax_gen, U_smt_c), x_ax_gen))))
    # Cls⊕
    ...
    current_axioms_list.append(ForAll(q_vars_ax_gen, Implies(And(in_S_Omega_f(x_ax_gen), in_S_Omega_f(y_ax_gen), cond_cls_add),
                                       Equals(add_func_symbol_to_use(x_ax_gen, y_ax_gen), sum_xy_ax_gen))))
    # Ovfl⊕
    ...
    current_axioms_list.append(ForAll(q_vars_ax_gen, Implies(And(in_S_Omega_f(x_ax_gen), in_S_Omega_f(y_ax_gen), cond_ovfl_add),
                                        Equals(add_func_symbol_to_use(x_ax_gen, y_ax_gen), U_smt_c))))
    # Ann⊗
    current_axioms_list.append(ForAll([x_ax_gen], Implies(in_S_Omega_f(x_ax_gen), Equals(mul_func_symbol_to_use(U_smt_c, x_ax_gen), U_smt_c))))
    current_axioms_list.append(ForAll([x_ax_gen], Implies(in_S_Omega_f(x_ax_gen), Equals(mul_func_symbol_to_use(x_ax_gen, U_smt_c), U_smt_c))))
    # Cls⊗
    ...
    current_axioms_list.append(ForAll(q_vars_ax_gen, Implies(And(in_S_Omega_f(x_ax_gen), in_S_Omega_f(y_ax_gen), cond_cls_mul),
                                       Equals(mul_func_symbol_to_use(x_ax_gen, y_ax_gen), prod_xy_ax_gen))))
    # Ovfl⊗
    ...
    current_axioms_list.append(ForAll(q_vars_ax_gen, Implies(And(in_S_Omega_f(x_ax_gen), in_S_Omega_f(y_ax_gen), cond_ovfl_mul),
                                        Equals(mul_func_symbol_to_use(x_ax_gen, y_ax_gen), U_smt_c))))
...
def run_gem4_tests(Omega_val: int, s: Solver, add_op_func_callable: Callable, U_smt_const: object,
    ...
    overall_gem4_passed = True # Initialize for the whole Gem 4
    
    # --- Gem 4-A: Classify Loop Properties ---
    print("\nGem 4-A: Checking Loop Properties for ⊕...")
    x_lp_q = Symbol(f"x_lp_q_O{Omega_val}", INT)
    y_lp_q = Symbol(f"y_lp_q_O{Omega_val}", INT)
    
    # Store result of Gem 4-A checks
    gem4a_passed_current_checks = True
    ...
    if not check_smt(s, Not(prop_left_inv_exists), False, "⊕ Left Inverse Existence (Loop property)"): 
        gem4a_passed_current_checks = False
    ...
    if gem4a_passed_current_checks: # Only print success if both checks in 4-A passed
        print(f"   ⊕ (for Ω={Omega_val}) confirmed consistent with Commutative (prev. proven), Power-Associative Loop with Inverses.")

    # --- Gem 4-B: Associator Defect Analysis ---
    # Initialize overall_gem4b_passed for this section
    overall_gem4b_passed_current_section = True 
    ...
    if not check_smt(s, exists_defect_triplet, expected_sat_defect, f"⊕ Non-Associative Triplet Exists (Expect SAT for Ω≥3)"): 
        overall_gem4b_passed_current_section = False
        overall_gem4_passed = False
    ...
    if not check_smt(s, Not(defect_claim_condition), expected_sat_for_not_defect_claim, 
                     f"⊕ Defect Claim (associative if a≠U & b≠U) - Expect {'SAT (claim is false)' if expected_sat_for_not_defect_claim else 'UNSAT (claim is true)'}"): 
        overall_gem4b_passed_current_section = False
        overall_gem4_passed = False
    ...
    x_nuc_q = Symbol(f"x_nuc_q_O{Omega_val}", INT); y_nuc_q = Symbol(f"y_nuc_q_O{Omega_val}", INT) 
    
    is_in_nucleus_prop_for_n = ForAll([x_nuc_q, y_nuc_q], Implies(
                                And(in_S_Omega_pred(x_nuc_q), in_S_Omega_pred(y_nuc_q)), 
                                Equals(add_op_func_callable(add_op_func_callable(n_nuc_sym, x_nuc_q), y_nuc_q), 
                                       add_op_func_callable(n_nuc_sym, add_op_func_callable(x_nuc_q, y_nuc_q)))))
    
    current_nucleus_passed_expectation = True
    for k_val_iter in range(Omega_val):
        k_smt_iter = Int(k_val_iter)
        prop_k_is_nucleus = Implies(in_S_Omega_pred(k_smt_iter), 
                                    is_in_nucleus_prop_for_n.substitute({n_nuc_sym: k_smt_iter}))
        
        s.push()
        s.add_assertion(Not(prop_k_is_nucleus))
        is_provably_in_nucleus = not s.solve() 
        s.pop()

        expected_to_be_in_nucleus = (Omega_val <= 2) or (k_val_iter == 0)

        if is_provably_in_nucleus:
            nucleus_set_py.append(k_val_iter)
            print(f"    Confirmed: {k_val_iter} IS in ⊕-nucleus.")
            if not expected_to_be_in_nucleus:
                print(f"    >>> WARNING: {k_val_iter} was found IN nucleus, but NOT expected for Ω={Omega_val}.")
                current_nucleus_passed_expectation = False
        else:
            print(f"    Confirmed: {k_val_iter} is NOT in ⊕-nucleus.")
            if expected_to_be_in_nucleus:
                print(f"    >>> WARNING: {k_val_iter} was NOT found in nucleus, but WAS expected for Ω={Omega_val}.")
                current_nucleus_passed_expectation = False
    
    if not current_nucleus_passed_expectation:
        overall_gem4b_passed_current_section = False
        overall_gem4_passed = False

    print(f"   Nucleus Elements for ⊕ (Ω={Omega_val}): {nucleus_set_py}")
    print(f"   Nucleus Size = {len(nucleus_set_py)}")
    expected_nucleus_size = Omega_val if Omega_val <= 2 else 1 
    if len(nucleus_set_py) != expected_nucleus_size:
        print(f"   WARNING: ⊕-Nucleus size {len(nucleus_set_py)} does not match expectation ({expected_nucleus_size}).")
        overall_gem4b_passed_current_section = False # Already captured by current_nucleus_passed_expectation
        overall_gem4_passed = False
    else:
        print(f"   ⊕-Nucleus size matches expectation ({expected_nucleus_size}).")
    
    return overall_gem4_passed # Return the overall pass status for Gem 4
...
def run_gem5_tests(Omega_val: int, s: Solver, add_op_func_callable: Callable, mul_op_func_callable: Callable, 
    ...
    if not check_smt(s, Not(prop_closure_add_u1t), False, "  {U,1,Ω-1} Closure under ⊕"): overall_gem5_passed=False
    ...
    if not check_smt(s, Not(prop_closure_mul_u1t), False, "  {U,1,Ω-1} Closure under ⊗"): overall_gem5_passed=False
    ...
    if not check_smt(s, Not(prop_assoc_add_sub_u1t), True, f"  {{U,1,Ω-1}} ⊕-Non-Associativity (Expect SAT for Ω≥3)"): overall_gem5_passed=False
    ...
    if not check_smt(s, Not(prop_distrib_sub_u1t), True, f"  {{U,1,Ω-1}} Non-Distributivity (Expect SAT for Ω≥3)"): overall_gem5_passed=False
    print(f"   Overall for {{U,1,Ω-1}} at Ω={Omega_val}: {'Test results align with expectations' if overall_gem5_passed else 'Some test results FAILED against expectation'}")
    ...
    if Omega_val <= 2:
        ...
        if check_smt(s, Not(prop_distrib_global), True, f"  Global Non-Distributivity (Ω={Omega_val} ≥ 3)"):
            ...
            if Omega_val == 3: # Example check for a known {U,k} ring from GIT
                ...
                # This {U,2} for Omega=3 should be distributive as it's an F2-shadow ring
...
def prove_for_all_gems(Omega_val: int):
    ...
    with Solver(name="z3") as s: # No specific options
        s.push()
        ...
        if Omega_val<=2:
            ...
            if Omega_val>=3:
                ...
                if om1v>=1 and (Omega_val ==1 or 1<Omega_val) : # Ensure 'one' is valid if Omega_val > 1
                    om1=Int(om1v)
       ...
       # Get the list of axiom formulas for these new function symbols
        # get_avca_axiom_formulas returns: U_smt, is_DFI_f, in_S_Omega_f, current_axioms_list
        # We only need the list of axioms for add2_f_sym_uniq and mul2_f_sym_uniq.
        # The U_smt, is_DFI_f, in_S_Omega_f would be identical to the main ones since Omega_val is the same.
