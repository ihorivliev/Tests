...
def smt_test_zero_divisors_mul(current_omega_py: int):
    ...
    if not all([a,b,res_ab, ZU_const_sym, omega_smt_node]): return # SMT init check

    setup_formulas = get_smt_unio_constant_constraints(ZU_const_sym, AS_const_sym, omega_smt_node) # type: ignore
    setup_formulas.extend([
        # a and b must be DFI
        Equals(a["is_finite"], TRUE()), # type: ignore
        Equals(b["is_finite"], TRUE()), # type: ignore
    ...
    # Property: a*b = ZU (algebraic Unio), and a,b are DFI (so not Unio)
    # The DFI constraint on a,b already means they are not Unio.
    property_to_verify = Equals(res_ab["is_zero"], TRUE()) # type: ignore 
    
    expected_to_hold = (current_omega_py >= 3) # Zero divisors exist iff Omega >= 3

    prove_or_find_counterexample_smt(property_name + f" (Expect: {'Exist' if expected_to_hold else 'Not Exist'})", 
                                     current_omega_py, setup_formulas, property_to_verify, # type: ignore
                                     [a,b,res_ab, ZU_const_sym], # type: ignore
                                     expect_property_to_hold=expected_to_hold, 
                                     is_existential=True)
...
def smt_test_multiplicative_cancellation(current_omega_py: int):
    ...
    c_is_not_unio = Equals(c["is_finite"], TRUE()) # type: ignore
