...
def avc_div_smt_logic(a: Dict[str, FNode], b: Dict[str, FNode],
                      res: Dict[str, FNode], omega_smt_node: FNode) -> FNode:
    """
    SMT Logic for Core AVCA-Ω avc_div (⊘_v1.2B: Variant B refinement).
    - Rule B1: Divisor b is Unio (any aspect) -> AREO_UNIO.
    - Rule B2: Else if Dividend a is Unio (any aspect) AND Divisor b is DFI -> Preserves Dividend's aspect.
    - Rule B3: Else (Both DFI) -> Standard quotient if exact & valid DFI; else AREO_UNIO.
    Corresponds to Python function avc_div_v1_2_variantB in AVCA Core DraftV3, Appendix A.
    ...
    # SMT formulas defining the state of 'res' for specific Unio aspects
    res_is_ZU_formula = And(res["is_zero"], 
                            Not(res["is_areo"]), 
                            Not(res["is_finite"]), 
                            Equals(res["val"], Int(0)))
    ...
    # --- Rule B1: Divisor 'b' is Unio ---
    # If b is Unio, result is AREO_UNIO, regardless of 'a'.
    rule_b1_condition = b_is_unio
    rule_b1_consequent = res_is_AU_formula

    # --- Rule B2: Divisor 'b' is DFI AND Dividend 'a' is Unio ---
    # This case is only reached if rule_b1_condition is False (i.e., b is DFI).
    rule_b2_condition = a_is_unio # b is already known to be DFI here
    # Result preserves dividend 'a's aspect
    rule_b2_consequent = Ite(a["is_zero"], 
                             res_is_ZU_formula,  # if a is ZERO_UNIO, res is ZERO_UNIO
                             res_is_AU_formula)  # if a is AREO_UNIO, res is AREO_UNIO
    
    # --- Rule B3: Both 'a' and 'b' are DFI ---
    # This case is only reached if rule_b1_condition is False (b is DFI) 
    # AND rule_b2_condition is False (a is DFI).
    # The DFI/DFI logic from your old script is largely correct for this rule.
    
    # Symbolic variables for quotient and remainder for DFI/DFI division
    # Ensure unique names if this function is called multiple times in one SMT context for a single property
    # (though typically one op per property part). PySMT handles symbol uniqueness usually.
    # Using res['name'] ensures some uniqueness based on the result variable's prefix.
    q_sym = Symbol(f"{res['name']}_q_div_b3", SMT_INT_TYPE)
    r_sym = Symbol(f"{res['name']}_r_div_b3", SMT_INT_TYPE)

    # Euclidean division definition: a = q*b + r, where 0 <= r < |b|
    # Since b["val"] for DFI is always > 0, |b| is b["val"].
    euclidean_division_definition = And(Equals(a["val"], (q_sym * b["val"]) + r_sym),
                                        r_sym >= Int(0),
                                        r_sym < b["val"])
    
    # Condition for a valid DFI quotient: remainder is 0, and 1 <= quotient < Omega
    quotient_is_valid_dfi_cond = And(Equals(r_sym, Int(0)),
                                     q_sym >= Int(1),
                                     q_sym < omega_smt_node)
    
    # Result 'res' is DFI quotient:
    res_is_FP_quotient_formula = And(Not(res["is_zero"]), 
                                     Not(res["is_areo"]), 
                                     res["is_finite"], 
                                     Equals(res["val"], q_sym))

    # Logic for Rule B3 (DFI / DFI)
    # This must include the Euclidean division constraints when this branch is active.
    rule_b3_consequent = And(
        euclidean_division_definition, # q_sym and r_sym are defined by a["val"] and b["val"]
        Ite(
            quotient_is_valid_dfi_cond,
            res_is_FP_quotient_formula,   # If valid DFI quotient, result is the quotient
            res_is_AU_formula             # Else (non-exact, q=0, q>=Omega), result is AREO_UNIO
        )
    )

    # Combine rules with priority using nested Ite (If-Then-Else)
    # Priority: Rule B1 > Rule B2 > Rule B3
    return Ite(rule_b1_condition,    # If divisor b is Unio
               rule_b1_consequent,   # Then result is AREO_UNIO
               Ite(rule_b2_condition,  # Else (b is DFI), if dividend a is Unio
                   rule_b2_consequent, # Then result preserves a's aspect
                   rule_b3_consequent    # Else (a is DFI, b is DFI), apply DFI/DFI logic
               )
              )
