# Script Name: AVC_General_Subtraction_DirectSpan_Test.py
...
# --- Configuration ---
# Requirement 4, Step 2 (Extended to Direct Span): Axiomatization and Verification of General Subtraction
...
def define_smart_raw_add_canonical_result(i1_canon_repr: Dict[str, Any], i2_canon_repr: Dict[str, Any],
    ...
    return define_operation_canonical_result(smart_raw_add_logic_builder, i1_canon_repr, i2_canon_repr, result_name_prefix, current_omega_eff_c_smt)

def define_smart_raw_sub_canonical_result(i1_canon_repr: Dict[str, Any], i2_canon_repr: Dict[str, Any],
                                           result_name_prefix: str, current_omega_eff_c_smt: FNode) -> Dict[str, Any]:
    return define_operation_canonical_result(smart_raw_sub_canonical_logic_builder, i1_canon_repr, i2_canon_repr, result_name_prefix, current_omega_eff_c_smt)
...
def map_local_val_to_canonical_repr_direct_span(
    ...
    k_L_val = Local_val_sym - PA_sym # 0-based relative value for canonical mapping

    # Kind assertions based on k_L_val relative to S_canonical_omega_sym (which is local span)
    ...
    # Value assertions for canonical representation
    ...
    # Ensure canonical finite value is within its bounds (0 < val < Omega_canonical)
...
def map_canonical_repr_to_local_val_direct_span(
    ...
    # If ZS_C, maps to Local PA_sym.
    # If AS_C, maps to Local PA_sym + S_canonical_omega_sym (which is PB_sym).
    # If Fp_C(v_C), maps to Local PA_sym + v_C.
...
def define_local_op_direct_span_result(
    ...
    Res_canon_repr = op_canonical_result_definer_func( 
        X_canon_repr, Y_canon_repr, f"{result_name_prefix_local}_resc", S_val_sym # Use S_val_sym as current_omega
    )
    ...
    debug_info = {"X_L_val_dbg": X_L_val_sym, "X_canon_repr_dbg": X_canon_repr,
                  ...
                  "Res_canon_repr_dbg": Res_canon_repr, "S_val_sym_dbg": S_val_sym, # Omega_eff_C is S_val_sym here
...
# ... (prove_or_find_counterexample - identical to previous script)
                ...
                for op_res_dict in op_result_dicts_for_debug:
                    if op_res_dict and isinstance(op_res_dict, dict) and "debug_info" in op_res_dict:
                        ...
                        if isinstance(debug_info, dict) and id(debug_info) not in printed_debug_info_ids:
                            ...
                            # For Direct Span, Omega_eff_C is S_val_dbg
                            o_eff_dbg_val_str = f"{solver.get_value(s_val_dbg)}" if s_val_dbg and s_val_dbg in solver.get_model() else "?"
                            s_val_str = o_eff_dbg_val_str 
...
if __name__ == "__main__":
    print("====== AVC General Subtraction Direct Span Test ======\n")

    P_A_val_sym = Symbol("PA_val_GSDS", solver_INT_TYPE)
    P_B_val_sym = Symbol("PB_val_GSDS", solver_INT_TYPE)
    
    A_L_val = Symbol("A_L_GSDS", solver_INT_TYPE)
    B_L_val = Symbol("B_L_GSDS", solver_INT_TYPE)
    Fp_L_val = Symbol("Fp_L_GSDS", solver_INT_TYPE) # For specific Fp tests

    A1_L_val = Symbol("A1_L_GSDS", solver_INT_TYPE)
    A2_L_val = Symbol("A2_L_GSDS", solver_INT_TYPE)
    B1_L_val = Symbol("B1_L_GSDS", solver_INT_TYPE)
    B2_L_val = Symbol("B2_L_GSDS", solver_INT_TYPE)

    for current_local_span_S_py in LOCAL_SPANS_TO_TEST:
        s = Solver(name="z3")
        current_local_span_S_smt = Int(current_local_span_S_py)
        print(f"\n\n===== DIRECT SPAN SUBTRACTION TESTS WITH Local Span S = {current_local_span_S_py} (Omega_eff_C = S) =====\n")
        ...
        all_local_inputs = [A_L_val, B_L_val, Fp_L_val, A1_L_val, A2_L_val, B1_L_val, B2_L_val]
        for inp_val_sym in all_local_inputs: 
            s.add_assertion(And(inp_val_sym >= P_A_val_sym, inp_val_sym <= P_B_val_sym))
        
        # --- GSDS-0: Well-Definedness ---
        s.push()
        prem_gsds0 = And(avc_equiv_local_val(A1_L_val, A2_L_val, P_A_val_sym, P_B_val_sym),
                         avc_equiv_local_val(B1_L_val, B2_L_val, P_A_val_sym, P_B_val_sym))
        res1_gsds0 = define_local_op_direct_span_result(define_smart_raw_sub_canonical_result, A1_L_val, B1_L_val, P_A_val_sym, P_B_val_sym, f"r1_gsds0_S{current_local_span_S_py}")
        res2_gsds0 = define_local_op_direct_span_result(define_smart_raw_sub_canonical_result, A2_L_val, B2_L_val, P_A_val_sym, P_B_val_sym, f"r2_gsds0_S{current_local_span_S_py}")
        conc_gsds0 = avc_equiv_local_val(res1_gsds0["val_L_sym"], res2_gsds0["val_L_sym"], P_A_val_sym, P_B_val_sym)
        setup_gsds0 = [prem_gsds0] + res1_gsds0["defining_assertions"] + res2_gsds0["defining_assertions"]
        prove_or_find_counterexample(s, f"GSDS-0 Well-Defined (S={current_local_span_S_py})", setup_gsds0, conc_gsds0, 
                                     model_vars_to_print=[P_A_val_sym, P_B_val_sym, A1_L_val, A2_L_val, B1_L_val, B2_L_val, res1_gsds0["val_L_sym"], res2_gsds0["val_L_sym"]],
                                     op_result_dicts_for_debug=[res1_gsds0, res2_gsds0])
        s.pop()

        # --- GSDS-1: A_L - A_L ~ ZS_L ---
        s.push()
        res_gsds1 = define_local_op_direct_span_result(define_smart_raw_sub_canonical_result, A_L_val, A_L_val, P_A_val_sym, P_B_val_sym, f"r_gsds1_S{current_local_span_S_py}")
        prop_gsds1 = is_local_ZS_val(res_gsds1["val_L_sym"], P_A_val_sym)
        prove_or_find_counterexample(s, f"GSDS-1 A_L-A_L ~ ZS_L (S={current_local_span_S_py})", res_gsds1["defining_assertions"], prop_gsds1,
                                     model_vars_to_print=[P_A_val_sym, P_B_val_sym, A_L_val, res_gsds1["val_L_sym"]], op_result_dicts_for_debug=[res_gsds1])
        s.pop()

        # --- GSDS-2: A_L - ZS_L ~ A_L ---
        s.push()
        res_gsds2 = define_local_op_direct_span_result(define_smart_raw_sub_canonical_result, A_L_val, P_A_val_sym, P_A_val_sym, P_B_val_sym, f"r_gsds2_S{current_local_span_S_py}")
        prop_gsds2 = avc_equiv_local_val(res_gsds2["val_L_sym"], A_L_val, P_A_val_sym, P_B_val_sym)
        prove_or_find_counterexample(s, f"GSDS-2 A_L-ZS_L ~ A_L (S={current_local_span_S_py})", res_gsds2["defining_assertions"], prop_gsds2,
                                     model_vars_to_print=[P_A_val_sym, P_B_val_sym, A_L_val, res_gsds2["val_L_sym"]], op_result_dicts_for_debug=[res_gsds2])
        s.pop()
        
        if current_local_span_S_py >= 2: # Fp_L can exist
            # --- GSDS-3: Fp_L(x) - AS_L ~ Fp_L(x) ---
            s.push()
            s.add_assertion(is_local_DFI_val(Fp_L_val, P_A_val_sym, P_B_val_sym)) # Fp_L_val is DFI
            res_gsds3 = define_local_op_direct_span_result(define_smart_raw_sub_canonical_result, Fp_L_val, P_B_val_sym, P_A_val_sym, P_B_val_sym, f"r_gsds3_S{current_local_span_S_py}")
            prop_gsds3 = avc_equiv_local_val(res_gsds3["val_L_sym"], Fp_L_val, P_A_val_sym, P_B_val_sym)
            prove_or_find_counterexample(s, f"GSDS-3 Fp_L-AS_L ~ Fp_L (S={current_local_span_S_py})", res_gsds3["defining_assertions"], prop_gsds3,
                                         model_vars_to_print=[P_A_val_sym, P_B_val_sym, Fp_L_val, res_gsds3["val_L_sym"]], op_result_dicts_for_debug=[res_gsds3])
            s.pop()

            # --- GSDS-4: ZS_L - Fp_L ~ AS_L ---
            s.push()
            s.add_assertion(is_local_DFI_val(Fp_L_val, P_A_val_sym, P_B_val_sym)) # Fp_L_val is DFI
            res_gsds4 = define_local_op_direct_span_result(define_smart_raw_sub_canonical_result, P_A_val_sym, Fp_L_val, P_A_val_sym, P_B_val_sym, f"r_gsds4_S{current_local_span_S_py}")
            prop_gsds4 = is_local_AS_val(res_gsds4["val_L_sym"], P_B_val_sym)
            prove_or_find_counterexample(s, f"GSDS-4 ZS_L-Fp_L ~ AS_L (S={current_local_span_S_py})", res_gsds4["defining_assertions"], prop_gsds4,
                                         model_vars_to_print=[P_A_val_sym, P_B_val_sym, Fp_L_val, res_gsds4["val_L_sym"]], op_result_dicts_for_debug=[res_gsds4])
            s.pop()

            # --- GSDS-5: AS_L - Fp_L ~ AS_L ---
            s.push()
            s.add_assertion(is_local_DFI_val(Fp_L_val, P_A_val_sym, P_B_val_sym)) # Fp_L_val is DFI
            res_gsds5 = define_local_op_direct_span_result(define_smart_raw_sub_canonical_result, P_B_val_sym, Fp_L_val, P_A_val_sym, P_B_val_sym, f"r_gsds5_S{current_local_span_S_py}")
            prop_gsds5 = is_local_AS_val(res_gsds5["val_L_sym"], P_B_val_sym)
            prove_or_find_counterexample(s, f"GSDS-5 AS_L-Fp_L ~ AS_L (S={current_local_span_S_py})", res_gsds5["defining_assertions"], prop_gsds5,
                                         model_vars_to_print=[P_A_val_sym, P_B_val_sym, Fp_L_val, res_gsds5["val_L_sym"]], op_result_dicts_for_debug=[res_gsds5])
            s.pop()
        else: # S=1
            print(f"--- SKIPPING GSDS-3,4,5 for S=1 (No DFI for Fp_L) ---")

        # --- GSDS-6: Additive Cancellation 1: (A_L + B_L) - B_L ~ A_L ---
        s.push()
        sum_ab_gs6 = define_local_op_direct_span_result(define_smart_raw_add_canonical_result, A_L_val, B_L_val, P_A_val_sym, P_B_val_sym, f"sumAB_gsds6_S{current_local_span_S_py}")
        res_sub_gs6 = define_local_op_direct_span_result(define_smart_raw_sub_canonical_result, sum_ab_gs6["val_L_sym"], B_L_val, P_A_val_sym, P_B_val_sym, f"resSub_gsds6_S{current_local_span_S_py}")
        prop_gsds6 = avc_equiv_local_val(res_sub_gs6["val_L_sym"], A_L_val, P_A_val_sym, P_B_val_sym)
        setup_gsds6 = sum_ab_gs6["defining_assertions"] + res_sub_gs6["defining_assertions"]
        prove_or_find_counterexample(s, f"GSDS-6 (A_L+B_L)-B_L ~ A_L (S={current_local_span_S_py})", setup_gsds6, prop_gsds6,
                                     model_vars_to_print=[P_A_val_sym, P_B_val_sym, A_L_val, B_L_val, sum_ab_gs6["val_L_sym"], res_sub_gs6["val_L_sym"]],
                                     op_result_dicts_for_debug=[sum_ab_gs6, res_sub_gs6])
        s.pop()

        # --- GSDS-7: Additive Cancellation 2: (A_L - B_L) + B_L ~ A_L ---
        s.push()
        sub_ab_gs7 = define_local_op_direct_span_result(define_smart_raw_sub_canonical_result, A_L_val, B_L_val, P_A_val_sym, P_B_val_sym, f"subAB_gsds7_S{current_local_span_S_py}")
        res_add_gs7 = define_local_op_direct_span_result(define_smart_raw_add_canonical_result, sub_ab_gs7["val_L_sym"], B_L_val, P_A_val_sym, P_B_val_sym, f"resAdd_gsds7_S{current_local_span_S_py}")
        prop_gsds7 = avc_equiv_local_val(res_add_gs7["val_L_sym"], A_L_val, P_A_val_sym, P_B_val_sym)
        setup_gsds7 = sub_ab_gs7["defining_assertions"] + res_add_gs7["defining_assertions"]
        prove_or_find_counterexample(s, f"GSDS-7 (A_L-B_L)+B_L ~ A_L (S={current_local_span_S_py})", setup_gsds7, prop_gsds7,
                                     model_vars_to_print=[P_A_val_sym, P_B_val_sym, A_L_val, B_L_val, sub_ab_gs7["val_L_sym"], res_add_gs7["val_L_sym"]],
                                     op_result_dicts_for_debug=[sub_ab_gs7, res_add_gs7])
        s.pop()
        
        del s
        print("-" * 80)

    print("\n====== AVC General Subtraction Direct Span Test Complete ======")