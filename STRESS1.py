...
def run_native_foundational_tests(omega_values_to_test: List[int], omega_native_max_val: int):
    ...
    for omega_val in omega_values_to_test:
        ...
        if omega_val > omega_native_max_val:
            print(f"  Note: Exhaustive tests for Ω={omega_val} may be skipped if > OMEGA_NATIVE_MAX ({omega_native_max_val}).")
    ...
    for omega_val, results in native_test_results_summary.items():
    ...
    if native_test_failures_details:
        ...
        for failure in native_test_failures_details:
    ...
    if total_failed_native == 0 and total_passed_native > 0 :
        ...
        print("⚪ All native foundational tests were SKIPPED (likely due to OMEGA_NATIVE_MAX).")
