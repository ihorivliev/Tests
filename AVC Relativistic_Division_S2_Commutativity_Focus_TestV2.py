...
def avc_equiv_local_val(X1_L_val_sym: FNode, X2_L_val_sym: FNode, 
    ...
    return Or(
        And(is_X1_L_ZS, is_X2_L_ZS), And(is_X1_L_AS, is_X2_L_AS),
        And(is_X1_L_ZS, is_X2_L_AS), And(is_X1_L_AS, is_X2_L_ZS),
...
def is_local_ZS_val(val_L_sym: FNode, P_A_val_sym: FNode) -> FNode: # Renamed from previous script for clarity
    return Equals(val_L_sym, P_A_val_sym)

def is_local_AS_val(val_L_sym: FNode, P_B_val_sym: FNode) -> FNode: # Renamed
    return Equals(val_L_sym, P_B_val_sym)

def is_local_DFI_val(val_L_sym: FNode, P_A_val_sym: FNode, P_B_val_sym: FNode) -> FNode: # Renamed
    return And(val_L_sym > P_A_val_sym, val_L_sym < P_B_val_sym)

def _apply_forced_kind_to_canonical_repr(assertions_list: List[FNode], 
                                        canon_repr: Dict[str, Any], 
                                        local_val_sym: FNode, # Original local value
                                        pa_val_sym: FNode,    # Local frame P_A
                                        s_val_sym: FNode,     # Local span S (Canonical Omega)
                                        forced_kind: str):
    """Helper to explicitly set canonical representation based on forced kind."""
    val_for_canon = local_val_sym - pa_val_sym
    if forced_kind == "ZS":
        assertions_list.append(canon_repr["is_zero"])
        assertions_list.append(Not(canon_repr["is_areo"]))
        assertions_list.append(Not(canon_repr["is_finite"]))
    elif forced_kind == "AS":
        assertions_list.append(canon_repr["is_areo"])
        assertions_list.append(Equals(canon_repr["val"], s_val_sym))
        assertions_list.append(Not(canon_repr["is_zero"]))
        assertions_list.append(Not(canon_repr["is_finite"]))
    elif forced_kind == "DFI":
        assertions_list.append(canon_repr["is_finite"])
        assertions_list.append(Equals(canon_repr["val"], val_for_canon))
        # These constraints are crucial for DFI and ensure it's valid for the canonical frame [0,S]
        assertions_list.append(val_for_canon > Int(0))
        assertions_list.append(val_for_canon < s_val_sym) 
        assertions_list.append(Not(canon_repr["is_zero"]))
        assertions_list.append(Not(canon_repr["is_areo"]))
    else:
        raise ValueError(f"Invalid forced_kind: {forced_kind}")


def define_local_op_direct_span_result_hardcore( # Renamed for clarity
    ...
    X_L_kind_force: str, # Must be "ZS", "AS", or "DFI"
    Y_L_kind_force: str  # Must be "ZS", "AS", or "DFI"
) -> Dict[str, Any]:
    ...
    # Apply forced kinds directly and explicitly
    _apply_forced_kind_to_canonical_repr(all_defining_assertions, X_canon_repr, X_L_val_sym, P_A_val_sym, current_S_val_sym, X_L_kind_force)
    _apply_forced_kind_to_canonical_repr(all_defining_assertions, Y_canon_repr, Y_L_val_sym, P_A_val_sym, current_S_val_sym, Y_L_kind_force)
    ...
    debug_info = {
        "X_L_val_dbg": X_L_val_sym, "X_canon_repr_dbg": X_canon_repr, "X_L_kind_forced_dbg": X_L_kind_force,
        "Y_L_val_dbg": Y_L_val_sym, "Y_canon_repr_dbg": Y_canon_repr, "Y_L_kind_forced_dbg": Y_L_kind_force,
                ...
                for op_res_dict in op_result_dicts_for_debug:
                    if op_res_dict and isinstance(op_res_dict, dict) and "debug_info" in op_res_dict:
                        ...
                        if isinstance(debug_info, dict) and id(debug_info) not in printed_debug_info_ids: 
                            ...
                            for repr_key_prefix_tuple in [("X", "X_L_kind_forced_dbg"), ("Y", "Y_L_kind_forced_dbg"), ("Res", None)]:
                                repr_key = f"{repr_key_prefix_tuple[0]}_canon_repr_dbg"
                                forced_kind_key = repr_key_prefix_tuple[1]
                                
                                repr_dict_dbg = debug_info.get(repr_key)
                                forced_kind_str = f"(Forced Local Kind: {debug_info.get(forced_kind_key, 'N/A')})" if forced_kind_key else ""
                                ...
                                if repr_dict_dbg and isinstance(repr_dict_dbg, dict) and "name" in repr_dict_dbg:
                                    ...
                                    print(f"      {repr_dict_dbg['name']} (Canon) {forced_kind_str}: {is_z_str_node}, {is_a_str_node}, {is_f_str_node}, {val_str_node}")
    solver.pop() 
    print("-" * 70)
...
if __name__ == "__main__":
    print("====== AVC Relativistic Division S=2 Commutativity Hardcore Test V2 ======\n")

    s = Solver(name="z3")
    current_S_smt = Int(LOCAL_SPAN_S_PY) # Fixed Span S=2

    PA_val = Symbol("PA_val_s2h2", solver_INT_TYPE) 
    PB_val = Symbol("PB_val_s2h2", solver_INT_TYPE)
    
    s.add_assertion(Equals(PA_val, Int(DEFAULT_P_A_OFFSET)))
    s.add_assertion(Equals(PB_val, PA_val + current_S_smt))

    # --- Test Case 1: DFI_L / ZS_L vs ZS_L / DFI_L ---
    La_val_c1 = PA_val + Int(1) # Represents Local DFI at P_A + 1
    Lb_val_c1 = PA_val       # Represents Local ZS  at P_A

    print(f"--- Testing RD-1 Commutativity S=2 - V2 HARDCORE Scenario: DFI_L / ZS_L ---")
    s.push()
    # Define DFI_L / ZS_L
    res_dfi_div_zs_dict = define_local_op_direct_span_result_hardcore(
        define_raw_div_canonical_result, 
        La_val_c1, Lb_val_c1, 
        PA_val, PB_val, "s2h2_dfi_div_zs",
        X_L_kind_force="DFI", Y_L_kind_force="ZS" 
    )
    # Define ZS_L / DFI_L
    res_zs_div_dfi_dict = define_local_op_direct_span_result_hardcore(
        define_raw_div_canonical_result,
        Lb_val_c1, La_val_c1, 
        PA_val, PB_val, "s2h2_zs_div_dfi",
        X_L_kind_force="ZS", Y_L_kind_force="DFI" 
    )
    property_comm_c1 = avc_equiv_local_val(
        res_dfi_div_zs_dict["val_L_sym"], res_zs_div_dfi_dict["val_L_sym"], PA_val, PB_val
    )
    all_assertions_c1 = res_dfi_div_zs_dict["defining_assertions"] + res_zs_div_dfi_dict["defining_assertions"]
    
    # For printing the concrete local values in the model if it fails
    La_print_c1 = Symbol("La_c1_val",solver_INT_TYPE); s.add_assertion(Equals(La_print_c1, La_val_c1))
    Lb_print_c1 = Symbol("Lb_c1_val",solver_INT_TYPE); s.add_assertion(Equals(Lb_print_c1, Lb_val_c1))

    prove_or_find_counterexample(s, f"RD-1 Commutativity S=2 V2 Hardcore - DFI_L / ZS_L",
                                 all_assertions_c1, property_comm_c1, 
                                 model_vars_to_print=[PA_val, PB_val, La_print_c1, Lb_print_c1, 
                                                      res_dfi_div_zs_dict["val_L_sym"], res_zs_div_dfi_dict["val_L_sym"]],
                                 op_result_dicts_for_debug=[res_dfi_div_zs_dict, res_zs_div_dfi_dict])
    s.pop()

    # --- Test Case 2: DFI_L / AS_L vs AS_L / DFI_L ---
    La_val_c2 = PA_val + Int(1) # Local DFI
    Lb_val_c2 = PB_val       # Local AS

    print(f"\n--- Testing RD-1 Commutativity S=2 - V2 HARDCORE Scenario: DFI_L / AS_L ---")
    s.push()
    res_dfi_div_as_dict = define_local_op_direct_span_result_hardcore(
        define_raw_div_canonical_result, La_val_c2, Lb_val_c2, PA_val, PB_val, "s2h2_dfi_div_as",
        X_L_kind_force="DFI", Y_L_kind_force="AS"
    )
    res_as_div_dfi_dict = define_local_op_direct_span_result_hardcore(
        define_raw_div_canonical_result, Lb_val_c2, La_val_c2, PA_val, PB_val, "s2h2_as_div_dfi",
        X_L_kind_force="AS", Y_L_kind_force="DFI"
    )
    property_comm_c2 = avc_equiv_local_val(
        res_dfi_div_as_dict["val_L_sym"], res_as_div_dfi_dict["val_L_sym"], PA_val, PB_val
    )
    all_assertions_c2 = res_dfi_div_as_dict["defining_assertions"] + res_as_div_dfi_dict["defining_assertions"]

    La_print_c2 = Symbol("La_c2_val",solver_INT_TYPE); s.add_assertion(Equals(La_print_c2, La_val_c2))
    Lb_print_c2 = Symbol("Lb_c2_val",solver_INT_TYPE); s.add_assertion(Equals(Lb_print_c2, Lb_val_c2))

    prove_or_find_counterexample(s, f"RD-1 Commutativity S=2 V2 Hardcore - DFI_L / AS_L",
                                 all_assertions_c2, property_comm_c2,
                                 model_vars_to_print=[PA_val, PB_val, La_print_c2, Lb_print_c2,
                                                      res_dfi_div_as_dict["val_L_sym"], res_as_div_dfi_dict["val_L_sym"]],
                                 op_result_dicts_for_debug=[res_dfi_div_as_dict, res_as_div_dfi_dict])
    s.pop()

    # --- Test Case 3: ZS_L / AS_L vs AS_L / ZS_L ---
    La_val_c3 = PA_val # Local ZS
    Lb_val_c3 = PB_val # Local AS

    print(f"\n--- Testing RD-1 Commutativity S=2 - V2 HARDCORE Scenario: ZS_L / AS_L ---")
    s.push()
    res_zs_div_as_dict = define_local_op_direct_span_result_hardcore(
        define_raw_div_canonical_result, La_val_c3, Lb_val_c3, PA_val, PB_val, "s2h2_zs_div_as",
        X_L_kind_force="ZS", Y_L_kind_force="AS"
    )
    res_as_div_zs_dict = define_local_op_direct_span_result_hardcore(
        define_raw_div_canonical_result, Lb_val_c3, La_val_c3, PA_val, PB_val, "s2h2_as_div_zs",
        X_L_kind_force="AS", Y_L_kind_force="ZS"
    )
    property_comm_c3 = avc_equiv_local_val(
        res_zs_div_as_dict["val_L_sym"], res_as_div_zs_dict["val_L_sym"], PA_val, PB_val
    )
    all_assertions_c3 = res_zs_div_as_dict["defining_assertions"] + res_as_div_zs_dict["defining_assertions"]
    
    La_print_c3 = Symbol("La_c3_val",solver_INT_TYPE); s.add_assertion(Equals(La_print_c3, La_val_c3))
    Lb_print_c3 = Symbol("Lb_c3_val",solver_INT_TYPE); s.add_assertion(Equals(Lb_print_c3, Lb_val_c3))

    prove_or_find_counterexample(s, f"RD-1 Commutativity S=2 V2 Hardcore - ZS_L / AS_L",
                                 all_assertions_c3, property_comm_c3,
                                 model_vars_to_print=[PA_val, PB_val, La_print_c3, Lb_print_c3,
                                                      res_zs_div_as_dict["val_L_sym"], res_as_div_zs_dict["val_L_sym"]],
                                 op_result_dicts_for_debug=[res_zs_div_as_dict, res_as_div_zs_dict])
    s.pop()
    
    del s
    print("\n====== AVC Relativistic Division S=2 Commutativity Hardcore Test V2 Complete ======")