# Script Name: AVC_CoreOps_DirectSpan_Sweep_Test.py
...
if __name__ == "__main__":
    print("====== AVC Core Operations Direct Span Sweep Test ======\n")

    P_A_val_sym = Symbol("PA_val_CODS", solver_INT_TYPE)
    P_B_val_sym = Symbol("PB_val_CODS", solver_INT_TYPE)
    
    A_L = Symbol("A_L_CODS", solver_INT_TYPE)
    B_L = Symbol("B_L_CODS", solver_INT_TYPE)
    C_L = Symbol("C_L_CODS", solver_INT_TYPE)
    
    A1_L_val = Symbol("A1_L_CODS", solver_INT_TYPE)
    A2_L_val = Symbol("A2_L_CODS", solver_INT_TYPE)
    B1_L_val = Symbol("B1_L_CODS", solver_INT_TYPE)
    B2_L_val = Symbol("B2_L_CODS", solver_INT_TYPE)
    
    Fp1_L_val = Symbol("Fp1_L_CODS", solver_INT_TYPE) 

    for current_local_span_S_py in LOCAL_SPANS_TO_TEST:
        s = Solver(name="z3")
        current_local_span_S_smt = Int(current_local_span_S_py) # This is Omega for Direct Span
        print(f"\n\n===== DIRECT SPAN CORE OPS TESTS WITH S = {current_local_span_S_py} (Omega_Canonical = S) =====\n")
        ...
        all_local_inputs = [A_L, B_L, C_L, A1_L_val, A2_L_val, B1_L_val, B2_L_val, Fp1_L_val]
        for inp_val_sym in all_local_inputs: 
            s.add_assertion(And(inp_val_sym >= P_A_val_sym, inp_val_sym <= P_B_val_sym))
        
        # --- ADDITION TESTS ---
        print(f"\n--- Testing ADDITION for S = {current_local_span_S_py} ---")
        # GADD-DS-0: Well-Definedness
        s.push()
        prem_gadd_ds0 = And(avc_equiv_local_val(A1_L_val, A2_L_val, P_A_val_sym, P_B_val_sym),
                            avc_equiv_local_val(B1_L_val, B2_L_val, P_A_val_sym, P_B_val_sym))
        res1_gadd0 = define_local_op_direct_span_result(smart_raw_add_logic_builder, A1_L_val, B1_L_val, P_A_val_sym, P_B_val_sym, f"r1_gadd0_S{current_local_span_S_py}")
        res2_gadd0 = define_local_op_direct_span_result(smart_raw_add_logic_builder, A2_L_val, B2_L_val, P_A_val_sym, P_B_val_sym, f"r2_gadd0_S{current_local_span_S_py}")
        conc_gadd0 = avc_equiv_local_val(res1_gadd0["val_L_sym"], res2_gadd0["val_L_sym"], P_A_val_sym, P_B_val_sym)
        setup_gadd0 = [prem_gadd_ds0] + res1_gadd0["defining_assertions"] + res2_gadd0["defining_assertions"]
        prove_or_find_counterexample(s, f"GADD-DS-0 Well-Defined (S={current_local_span_S_py})", setup_gadd0, conc_gadd0, 
                                     model_vars_to_print=[A1_L_val, A2_L_val, B1_L_val, B2_L_val, res1_gadd0, res2_gadd0],
                                     op_result_dicts_for_debug=[res1_gadd0, res2_gadd0])
        s.pop()

        # GADD-DS-1: ZS_L + A_L ~ A_L 
        s.push()
        res_gadd1 = define_local_op_direct_span_result(smart_raw_add_logic_builder, P_A_val_sym, A_L, P_A_val_sym, P_B_val_sym, f"r_gadd1_S{current_local_span_S_py}")
        prop_gadd1 = avc_equiv_local_val(res_gadd1["val_L_sym"], A_L, P_A_val_sym, P_B_val_sym)
        prove_or_find_counterexample(s, f"GADD-DS-1 ZS_L + A_L ~ A_L (S={current_local_span_S_py})", res_gadd1["defining_assertions"], prop_gadd1,
                                     op_result_dicts_for_debug=[res_gadd1])
        s.pop()

        # GADD-DS-5: Commutativity A_L + B_L ~ B_L + A_L
        s.push()
        res1_gadd5 = define_local_op_direct_span_result(smart_raw_add_logic_builder, A_L, B_L, P_A_val_sym, P_B_val_sym, f"r1_gadd5_S{current_local_span_S_py}")
        res2_gadd5 = define_local_op_direct_span_result(smart_raw_add_logic_builder, B_L, A_L, P_A_val_sym, P_B_val_sym, f"r2_gadd5_S{current_local_span_S_py}")
        ...
        prove_or_find_counterexample(s, f"GADD-DS-5 Commutativity (S={current_local_span_S_py})", setup_gadd5, prop_gadd5,
                                     op_result_dicts_for_debug=[res1_gadd5, res2_gadd5])
        s.pop()
        
        # GADD-DS-6: Associativity (A_L + B_L) + C_L ~ A_L + (B_L + C_L) (DFIs)
        s.push()
        if current_local_span_S_py >= 2: # Need DFIs for non-trivial test
            ...
            sum_ab_gadd6 = define_local_op_direct_span_result(smart_raw_add_logic_builder, A_L, B_L, P_A_val_sym, P_B_val_sym, f"sAB_gadd6_S{current_local_span_S_py}")
            lhs_gadd6 = define_local_op_direct_span_result(smart_raw_add_logic_builder, sum_ab_gadd6["val_L_sym"], C_L, P_A_val_sym, P_B_val_sym, f"lhs_gadd6_S{current_local_span_S_py}")
            sum_bc_gadd6 = define_local_op_direct_span_result(smart_raw_add_logic_builder, B_L, C_L, P_A_val_sym, P_B_val_sym, f"sBC_gadd6_S{current_local_span_S_py}")
            rhs_gadd6 = define_local_op_direct_span_result(smart_raw_add_logic_builder, A_L, sum_bc_gadd6["val_L_sym"], P_A_val_sym, P_B_val_sym, f"rhs_gadd6_S{current_local_span_S_py}")
            ...
            prove_or_find_counterexample(s, f"GADD-DS-6 Associativity (DFIs) (S={current_local_span_S_py})", setup_gadd6, prop_gadd6,
                                         model_vars_to_print=[A_L,B_L,C_L,lhs_gadd6,rhs_gadd6], op_result_dicts_for_debug=[sum_ab_gadd6,lhs_gadd6,sum_bc_gadd6,rhs_gadd6])
        else: # S=1, DFI inputs not possible
            print(f"--- SKIPPING GADD-DS-6 for S={current_local_span_S_py} (No DFI inputs) ---")
        s.pop()


        # --- MULTIPLICATION TESTS ---
        print(f"\n--- Testing MULTIPLICATION for S = {current_local_span_S_py} ---")
        # GMUL-DS-0: Well-Definedness
        s.push()
        prem_gmul_ds0 = And(avc_equiv_local_val(A1_L_val, A2_L_val, P_A_val_sym, P_B_val_sym),
                            avc_equiv_local_val(B1_L_val, B2_L_val, P_A_val_sym, P_B_val_sym))
        res1_gmul0 = define_local_op_direct_span_result(raw_mul_logic_builder, A1_L_val, B1_L_val, P_A_val_sym, P_B_val_sym, f"r1_gmul0_S{current_local_span_S_py}")
        res2_gmul0 = define_local_op_direct_span_result(raw_mul_logic_builder, A2_L_val, B2_L_val, P_A_val_sym, P_B_val_sym, f"r2_gmul0_S{current_local_span_S_py}")
        conc_gmul0 = avc_equiv_local_val(res1_gmul0["val_L_sym"], res2_gmul0["val_L_sym"], P_A_val_sym, P_B_val_sym)
        setup_gmul0 = [prem_gmul_ds0] + res1_gmul0["defining_assertions"] + res2_gmul0["defining_assertions"]
        prove_or_find_counterexample(s, f"GMUL-DS-0 Well-Defined (S={current_local_span_S_py})", setup_gmul0, conc_gmul0, 
                                     op_result_dicts_for_debug=[res1_gmul0, res2_gmul0])
        s.pop()

        # GMUL-DS-6: Associativity (A_L * B_L) * C_L ~ A_L * (B_L * C_L) (DFIs)
        s.push()
        if current_local_span_S_py >= 2:
            ...
            prod_ab_gmul6 = define_local_op_direct_span_result(raw_mul_logic_builder, A_L, B_L, P_A_val_sym, P_B_val_sym, f"pAB_gmul6_S{current_local_span_S_py}")
            lhs_gmul6 = define_local_op_direct_span_result(raw_mul_logic_builder, prod_ab_gmul6["val_L_sym"], C_L, P_A_val_sym, P_B_val_sym, f"lhs_gmul6_S{current_local_span_S_py}")
            prod_bc_gmul6 = define_local_op_direct_span_result(raw_mul_logic_builder, B_L, C_L, P_A_val_sym, P_B_val_sym, f"pBC_gmul6_S{current_local_span_S_py}")
            rhs_gmul6 = define_local_op_direct_span_result(raw_mul_logic_builder, A_L, prod_bc_gmul6["val_L_sym"], P_A_val_sym, P_B_val_sym, f"rhs_gmul6_S{current_local_span_S_py}")
            ...
            prove_or_find_counterexample(s, f"GMUL-DS-6 Associativity (DFIs) (S={current_local_span_S_py})", setup_gmul6, prop_gmul6,
                                         op_result_dicts_for_debug=[prod_ab_gmul6,lhs_gmul6,prod_bc_gmul6,rhs_gmul6])
        else:
            print(f"--- SKIPPING GMUL-DS-6 for S={current_local_span_S_py} (No DFI inputs) ---")
        s.pop()

        # --- DIVISION TESTS ---
        print(f"\n--- Testing DIVISION for S = {current_local_span_S_py} ---")
        # GDIV-DS-0: Well-Definedness
        s.push()
        prem_gdiv_ds0 = And(avc_equiv_local_val(A1_L_val, A2_L_val, P_A_val_sym, P_B_val_sym),
                            avc_equiv_local_val(B1_L_val, B2_L_val, P_A_val_sym, P_B_val_sym))
        res1_gdiv0 = define_local_op_direct_span_result(raw_div_logic_builder, A1_L_val, B1_L_val, P_A_val_sym, P_B_val_sym, f"r1_gdiv0_S{current_local_span_S_py}")
        res2_gdiv0 = define_local_op_direct_span_result(raw_div_logic_builder, A2_L_val, B2_L_val, P_A_val_sym, P_B_val_sym, f"r2_gdiv0_S{current_local_span_S_py}")
        conc_gdiv0 = avc_equiv_local_val(res1_gdiv0["val_L_sym"], res2_gdiv0["val_L_sym"], P_A_val_sym, P_B_val_sym)
        setup_gdiv0 = [prem_gdiv_ds0] + res1_gdiv0["defining_assertions"] + res2_gdiv0["defining_assertions"]
        prove_or_find_counterexample(s, f"GDIV-DS-0 Well-Defined (S={current_local_span_S_py})", setup_gdiv0, conc_gdiv0,
                                     op_result_dicts_for_debug=[res1_gdiv0, res2_gdiv0])
        s.pop()
        
        # GDIV-DS-6: Associativity (A_L / B_L) / C_L ~ A_L / (B_L / C_L) (DFIs)
        s.push()
        if current_local_span_S_py >= 2:
            s.add_assertion(is_local_DFI_val(A_L, P_A_val_sym, P_B_val_sym))
            s.add_assertion(is_local_DFI_val(B_L, P_A_val_sym, P_B_val_sym))
            ...
            div_ab_gdiv6 = define_local_op_direct_span_result(raw_div_logic_builder, A_L, B_L, P_A_val_sym, P_B_val_sym, f"dAB_gdiv6_S{current_local_span_S_py}")
            lhs_gdiv6 = define_local_op_direct_span_result(raw_div_logic_builder, div_ab_gdiv6["val_L_sym"], C_L, P_A_val_sym, P_B_val_sym, f"lhs_gdiv6_S{current_local_span_S_py}")
            div_bc_gdiv6 = define_local_op_direct_span_result(raw_div_logic_builder, B_L, C_L, P_A_val_sym, P_B_val_sym, f"dBC_gdiv6_S{current_local_span_S_py}")
            rhs_gdiv6 = define_local_op_direct_span_result(raw_div_logic_builder, A_L, div_bc_gdiv6["val_L_sym"], P_A_val_sym, P_B_val_sym, f"rhs_gdiv6_S{current_local_span_S_py}")
            ...
            prove_or_find_counterexample(s, f"GDIV-DS-6 Associativity (DFIs) (S={current_local_span_S_py})", setup_gdiv6, prop_gdiv6,
                                         op_result_dicts_for_debug=[div_ab_gdiv6,lhs_gdiv6,div_bc_gdiv6,rhs_gdiv6])
        else:
            print(f"--- SKIPPING GDIV-DS-6 for S={current_local_span_S_py} (No DFI inputs) ---")
        s.pop()

        # --- DISTRIBUTIVITY (Mul over Add) ---
        print(f"\n--- Testing DISTRIBUTIVITY for S = {current_local_span_S_py} ---")
        # GDA-DS-1: C_L * (A_L + B_L) ~ (C_L*A_L) + (C_L*B_L) (DFIs)
        s.push()
        if current_local_span_S_py >= 2:
            s.add_assertion(is_local_DFI_val(A_L, P_A_val_sym, P_B_val_sym))
            s.add_assertion(is_local_DFI_val(B_L, P_A_val_sym, P_B_val_sym))
            ...
            sum_ab_gda1 = define_local_op_direct_span_result(smart_raw_add_logic_builder, A_L, B_L, P_A_val_sym, P_B_val_sym, f"sAB_gda1_S{current_local_span_S_py}")
            lhs_gda1 = define_local_op_direct_span_result(raw_mul_logic_builder, C_L, sum_ab_gda1["val_L_sym"], P_A_val_sym, P_B_val_sym, f"lhs_gda1_S{current_local_span_S_py}")
            
            prod_ca_gda1 = define_local_op_direct_span_result(raw_mul_logic_builder, C_L, A_L, P_A_val_sym, P_B_val_sym, f"pCA_gda1_S{current_local_span_S_py}")
            prod_cb_gda1 = define_local_op_direct_span_result(raw_mul_logic_builder, C_L, B_L, P_A_val_sym, P_B_val_sym, f"pCB_gda1_S{current_local_span_S_py}")
            rhs_gda1 = define_local_op_direct_span_result(smart_raw_add_logic_builder, prod_ca_gda1["val_L_sym"], prod_cb_gda1["val_L_sym"], P_A_val_sym, P_B_val_sym, f"rhs_gda1_S{current_local_span_S_py}")
            prop_gda1 = avc_equiv_local_val(lhs_gda1["val_L_sym"], rhs_gda1["val_L_sym"], P_A_val_sym, P_B_val_sym)
            setup_gda1 = (sum_ab_gda1["defining_assertions"] + lhs_gda1["defining_assertions"] + 
                          prod_ca_gda1["defining_assertions"] + prod_cb_gda1["defining_assertions"] + rhs_gda1["defining_assertions"])
            prove_or_find_counterexample(s, f"GDA-DS-1 Distributivity (DFIs) (S={current_local_span_S_py})", setup_gda1, prop_gda1,
                                         op_result_dicts_for_debug=[sum_ab_gda1,lhs_gda1,prod_ca_gda1,prod_cb_gda1,rhs_gda1])
        else:
            print(f"--- SKIPPING GDA-DS-1 for S={current_local_span_S_py} (No DFI inputs) ---")
        s.pop()

        del s
        print("-" * 80)

    print("\n====== AVC Core Operations Direct Span Sweep Test Complete ======")