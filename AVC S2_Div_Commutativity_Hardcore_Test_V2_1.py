...
def avc_equiv_local_val(X1_L_val_sym: FNode, X2_L_val_sym: FNode, 
    ...
    # For S=2, DFI is P_A + 1. P_B = P_A + 2.
    is_X1_L_DFI = And(X1_L_val_sym > P_A_val_sym, X1_L_val_sym < P_B_val_sym, Equals(P_B_val_sym - P_A_val_sym, Int(2)))


    is_X2_L_ZS = Equals(X2_L_val_sym, P_A_val_sym)
    is_X2_L_AS = Equals(X2_L_val_sym, P_B_val_sym)
    is_X2_L_DFI = And(X2_L_val_sym > P_A_val_sym, X2_L_val_sym < P_B_val_sym, Equals(P_B_val_sym - P_A_val_sym, Int(2)))
...
def raw_div_logic_builder(i1: Dict[str, Any], i2: Dict[str, Any], 
    ...
    # When divisor i2 is Finite, its val > 0 is guaranteed by i2["constraints"]
...
# --- Phase 3: Local Operation Definition (Direct Span S as Omega_C) ---
def _apply_forced_kind_to_canonical_repr(assertions_list: List[FNode], 
    ...
    if forced_kind == "ZS":
        ...
        # Crucial constraints for this val_for_canon to be a valid canonical DFI
...
def define_local_op_direct_span_result_hardcore( 
                    ...
                    if op_res_dict and isinstance(op_res_dict, dict) and "debug_info" in op_res_dict:
                        ...
                        if isinstance(debug_info, dict) and id(debug_info) not in printed_debug_info_ids: 
                            ...
                            for repr_key_prefix_tuple in [("X", "X_L_kind_forced_dbg"), ("Y", "Y_L_kind_forced_dbg"), ("Res", None)]:
                                ...
                                forced_kind_str = f"(Forced Local Kind: {debug_info.get(forced_kind_key, 'N/A')})" if forced_kind_key and debug_info.get(forced_kind_key) else ""
...
if __name__ == "__main__":
    ...
    s = Solver(name="z3")
    current_S_smt = Int(LOCAL_SPAN_S_PY) 
    ...
    # For S=2 (e.g. P_A=0, P_B=2), DFI_L is value P_A+1 (1). ZS_L is value P_A (0).
    La_val_c1 = PA_val + Int(1) 
    Lb_val_c1 = PA_val       

    print(f"--- Testing RD-1 Commutativity S=2 - V2 HARDCORE Scenario: DFI_L / ZS_L ---")
    s.push()
    
    op_name_ab_c1 = f"s2h2_dfi_div_zs"
    op_name_ba_c1 = f"s2h2_zs_div_dfi"

    res_dfi_div_zs_dict = define_local_op_direct_span_result_hardcore(
        define_raw_div_canonical_result, La_val_c1, Lb_val_c1, 
    ...
    res_zs_div_dfi_dict = define_local_op_direct_span_result_hardcore(
        define_raw_div_canonical_result, Lb_val_c1, La_val_c1, 
    ...
    La_print_c1 = Symbol("La_c1_val_s2h2",solver_INT_TYPE); s.add_assertion(Equals(La_print_c1, La_val_c1))
    Lb_print_c1 = Symbol("Lb_c1_val_s2h2",solver_INT_TYPE); s.add_assertion(Equals(Lb_print_c1, Lb_val_c1))
    ...
    La_val_c2 = PA_val + Int(1) 
    Lb_val_c2 = PB_val       

    print(f"\n--- Testing RD-1 Commutativity S=2 - V2 HARDCORE Scenario: DFI_L / AS_L ---")
    s.push()
    op_name_ab_c2 = f"s2h2_dfi_div_as"
    op_name_ba_c2 = f"s2h2_as_div_dfi"

    res_dfi_div_as_dict = define_local_op_direct_span_result_hardcore(
        define_raw_div_canonical_result, La_val_c2, Lb_val_c2, PA_val, PB_val, op_name_ab_c2,
        X_L_kind_force="DFI", Y_L_kind_force="AS"
    )
    res_as_div_dfi_dict = define_local_op_direct_span_result_hardcore(
        define_raw_div_canonical_result, Lb_val_c2, La_val_c2, PA_val, PB_val, op_name_ba_c2,
    ...
    La_print_c2 = Symbol("La_c2_val_s2h2",solver_INT_TYPE); s.add_assertion(Equals(La_print_c2, La_val_c2))
    Lb_print_c2 = Symbol("Lb_c2_val_s2h2",solver_INT_TYPE); s.add_assertion(Equals(Lb_print_c2, Lb_val_c2))
    ...
    s.push()
    op_name_ab_c3 = f"s2h2_zs_div_as"
    op_name_ba_c3 = f"s2h2_as_div_zs"

    res_zs_div_as_dict = define_local_op_direct_span_result_hardcore(
        define_raw_div_canonical_result, La_val_c3, Lb_val_c3, PA_val, PB_val, op_name_ab_c3,
        X_L_kind_force="ZS", Y_L_kind_force="AS"
    )
    res_as_div_zs_dict = define_local_op_direct_span_result_hardcore(
        define_raw_div_canonical_result, Lb_val_c3, La_val_c3, PA_val, PB_val, op_name_ba_c3,
    ...
    La_print_c3 = Symbol("La_c3_val_s2h2",solver_INT_TYPE); s.add_assertion(Equals(La_print_c3, La_val_c3))
    Lb_print_c3 = Symbol("Lb_c3_val_s2h2",solver_INT_TYPE); s.add_assertion(Equals(Lb_print_c3, Lb_val_c3))
