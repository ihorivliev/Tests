...
def run_smt_foundational_tests(omega_values_to_test: List[int]):
    ...
    # Current: Add(c,a,id,inv_ex,inv_uq_fp1,inv_uq_fp2=exist), Mul(c,a,zd), Distrib, Cancel(a,m) = 11
    # New: Sub(id,stuck,noncomm,nonassoc) = 4
    #      Div(unio_interact(3 parts), dfi_cases(3 parts), round_trip(1 part)) = 7
    #      New Mul_ID (fp1_cond_id, fp1_not_univ_id) = 2
    #      New Add_Inv_Count_Specific (1 part for Omega=3) = 1 (conditionally)
    # Total approx 11 + 4 + 7 + 2 + 1 = 25 (some are conditional on Omega)
    num_intended_smt_tests_approx = 25 
    ...
    for omega_val in omega_values_to_test:
        ...
        smt_test_dfi_additive_inverse_count_specific_case(omega_val) # New

        # === Multiplication Properties ===
        smt_test_commutativity_mul(omega_val)         
        smt_test_associativity_mul(omega_val)         
        smt_test_zero_divisors_mul(omega_val) 
        smt_test_fp1_multiplicative_identity_for_dfi(omega_val) # New
        smt_test_fp1_not_universal_multiplicative_identity(omega_val) # New
        ...
        smt_test_subtraction_dfi_cases(omega_val) # New

        # === Division Properties ===
        smt_test_division_unio_interactions(omega_val) # New
        smt_test_division_dfi_cases(omega_val) # New
        smt_test_division_round_trip_constrained(omega_val) # New
