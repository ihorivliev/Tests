...
def test_H_C1_operational_uniqueness_O4(omega_py: int):
    ...
    # A3, A4, A5 for oplus_X (forces oplus_X = add_AVCA_Omega4_table)
    for i_py_key in range(omega_py):
        for j_py_key in range(omega_py):
            i_smt_key, j_smt_key = Int(i_py_key), Int(j_py_key)
            expected_add_res = Int(add_AVCA_Omega4_table[(i_py_key, j_py_key)])
            solver.add_assertion(Equals(res_add_X[(i_smt_key, j_smt_key)], expected_add_res))
    
    # A2_tensor (Commutativity of otimes_X)
    for v1_idx in range(omega_py):
        for v2_idx in range(v1_idx + 1, omega_py): # Assert for unique pairs i < j
            ...
            for k_val in s_omega_vals:
                ...
                lhs_final_val_assoc = Ite(Equals(ij_mul_X_sym, U_alg), res_mul_X[(U_alg, k_val)],
                                     ...
                                     Ite(Equals(ij_mul_X_sym, FP2_alg), res_mul_X[(FP2_alg, k_val)],
                                         res_mul_X[(FP3_alg, k_val)]))) # Fallback to FP3 if ij_mul_X is FP3
                ...
                rhs_final_val_assoc = Ite(Equals(jk_mul_X_sym, U_alg), res_mul_X[(i_val, U_alg)],
                                     ...
                                     Ite(Equals(jk_mul_X_sym, FP2_alg), res_mul_X[(i_val, FP2_alg)],
                                         res_mul_X[(i_val, FP3_alg)]))) # Fallback to FP3 if jk_mul_X is FP3
    ...
    for x_val in s_omega_vals: # U_alg as annihilator
        solver.add_assertion(Equals(res_mul_X[(U_alg, x_val)], U_alg))
        if x_val != U_alg : # Avoid redundant U_alg, U_alg if already covered by commutativity
    ...
    # Commutativity already asserted covers FP2_alg*FP1_alg etc.
    ...
    for i_py_key_diff in range(omega_py):
        for j_py_key_diff in range(omega_py):
            ...
            # oplus_X is already forced to be add_AVCA_Omega4_table by A3,A4,A5.
    ...
    solve_result = solver.solve() # Store the boolean result
    if not solve_result:
        ...
        # model = solver.get_model() # Model can be very large
        property_holds = False
