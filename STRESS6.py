...
def run_smt_foundational_tests(omega_values_to_test: List[int]):
    ...
    if not SMT_MODE_AVAILABLE:
        ...
        # Estimate number of SMT tests we intend to run
        num_intended_smt_tests = 11 # add(c,a,id,inv_ex,inv_uq), mul(c,a,zd), distrib, cancel(a,m)
        for omega_val in omega_values_to_test:
            _initialize_smt_omega_results(omega_val)
            smt_test_results_summary[omega_val]["skipped"] += num_intended_smt_tests
        return

    for omega_val in omega_values_to_test:
        print(f"\n--- SMT Tests for Ω = {omega_val} ---")
        if omega_val == 0: # Should not happen if list is curated
            print(f"  SKIPPING SMT Tests for invalid Ω = {omega_val}")
            _initialize_smt_omega_results(omega_val)
            smt_test_results_summary[omega_val]["skipped"] += 11 # Update count if necessary
            continue
