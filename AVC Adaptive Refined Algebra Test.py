...
# Using existing robust smart_raw_add_logic_builder and raw_mul_logic_builder
...
# --- Phase 3: Local Frame, Adaptive Omega, and Mappings (Copied and verified from previous script) ---
...
def map_local_to_adaptive_archetype_input_repr(
    ...
    val_assertions.append(
        Implies(arch_repr["is_finite"], 
            Ite(Equals(Omega_eff_C_sym, Int(1)), FALSE(), # Should be unreachable if arch_repr is finite
...
if __name__ == "__main__":
    ...
    # For general algebraic tests (La, Lb, Lc can be local ZS, AS, or DFI)
    La_val_sym = Symbol("La_val_refined", solver_INT_TYPE)
    Lb_val_sym = Symbol("Lb_val_refined", solver_INT_TYPE)
    Lc_val_sym = Symbol("Lc_val_refined", solver_INT_TYPE)

    # For well-definedness tests
    Li1_val_sym = Symbol("Li1_val_refined", solver_INT_TYPE)
    Lj1_val_sym = Symbol("Lj1_val_refined", solver_INT_TYPE)
    Li2_val_sym = Symbol("Li2_val_refined", solver_INT_TYPE)
    Lj2_val_sym = Symbol("Lj2_val_refined", solver_INT_TYPE)
    ...
    for current_local_span_S_py in OMEGA_VARIANTS_FOR_LOCAL_SPAN_S:
        ...
        # --- L0_Adapt_Refined: Sanity of avc_equiv_local (Basic checks) ---
        # (Covered implicitly by more complex tests; can be added explicitly if needed)

        # --- L1_Adapt_Refined: Well-Definedness of Local Adaptive Operations ---
        ...
        premise_L1AR = And(avc_equiv_local(Li1_val_sym, Li2_val_sym, P_A_val_sym, P_B_val_sym),
        ...
        prove_or_find_counterexample(s, f"L1.1AR Add respects avc_equiv_local (S={current_local_span_S_py})",
                                     [premise_L1AR] + op_L1AR_add_r1["defining_assertions"] + op_L1AR_add_r2["defining_assertions"], conc_L1AR_add,
                                     model_vars_to_print=[P_A_val_sym,P_B_val_sym,Li1_val_sym,Lj1_val_sym,Li2_val_sym,Lj2_val_sym,op_L1AR_add_r1["val_L_sym"],op_L1AR_add_r2["val_L_sym"]],
                                     op_result_dicts_for_debug=[op_L1AR_add_r1, op_L1AR_add_r2])
        ...
        prove_or_find_counterexample(s, f"L1.2AR Mul respects avc_equiv_local (S={current_local_span_S_py})",
                                     [premise_L1AR] + op_L1AR_mul_r1["defining_assertions"] + op_L1AR_mul_r2["defining_assertions"], conc_L1AR_mul,
                                     model_vars_to_print=[P_A_val_sym,P_B_val_sym,Li1_val_sym,Lj1_val_sym,Li2_val_sym,Lj2_val_sym,op_L1AR_mul_r1["val_L_sym"],op_L1AR_mul_r2["val_L_sym"]],
                                     op_result_dicts_for_debug=[op_L1AR_mul_r1, op_L1AR_mul_r2])
        s.pop()

        # --- L2_Adapt_Refined: Associativity of smart_raw_add_local_adaptive ---
        ...
        prove_or_find_counterexample(s, f"L2AR Add Assoc (S={current_local_span_S_py})", all_L2AR_asserts,
                                     avc_equiv_local(lhs_L2AR["val_L_sym"], rhs_L2AR["val_L_sym"], P_A_val_sym, P_B_val_sym),
                                     model_vars_to_print=[P_A_val_sym,P_B_val_sym,La_val_sym,Lb_val_sym,Lc_val_sym,sum_ab_L2AR["val_L_sym"],lhs_L2AR["val_L_sym"],sum_bc_L2AR["val_L_sym"],rhs_L2AR["val_L_sym"]],
                                     op_result_dicts_for_debug=[sum_ab_L2AR, lhs_L2AR, sum_bc_L2AR, rhs_L2AR])
        s.pop()

        # --- L3_Adapt_Refined: Associativity of raw_mul_local_adaptive ---
        ...
        prove_or_find_counterexample(s, f"L3AR Mul Assoc (S={current_local_span_S_py})", all_L3AR_asserts,
                                     avc_equiv_local(lhs_mul_L3AR["val_L_sym"], rhs_mul_L3AR["val_L_sym"], P_A_val_sym, P_B_val_sym),
                                     model_vars_to_print=[P_A_val_sym,P_B_val_sym,La_val_sym,Lb_val_sym,Lc_val_sym],
                                     op_result_dicts_for_debug=[prod_ab_L3AR, lhs_mul_L3AR, prod_bc_L3AR, rhs_mul_L3AR])
        s.pop()
        ...
        prove_or_find_counterexample(s, f"L4AR Distributivity (S={current_local_span_S_py})", all_L4AR_asserts,
                                     avc_equiv_local(lhs_dist_L4AR["val_L_sym"], rhs_dist_L4AR["val_L_sym"], P_A_val_sym, P_B_val_sym),
                                     model_vars_to_print=[P_A_val_sym,P_B_val_sym,La_val_sym,Lb_val_sym,Lc_val_sym,sum_bc_L4AR["val_L_sym"],lhs_dist_L4AR["val_L_sym"],prod_ab_L4AR["val_L_sym"],prod_ac_L4AR["val_L_sym"],rhs_dist_L4AR["val_L_sym"]],
                                     op_result_dicts_for_debug=op_results_L4AR_debug)
        s.pop()
        ...
        prove_or_find_counterexample(s, f"L5AR Additive Cancellation (S={current_local_span_S_py})",
                                     [premise_L5AR] + all_L5AR_asserts, conclusion_L5AR,
        ...
        prove_or_find_counterexample(s, f"L6AR Additive Idempotence La+La ~ La (S={current_local_span_S_py})",
                                     add_aa_L6AR["defining_assertions"], avc_equiv_local(add_aa_L6AR["val_L_sym"], La_val_sym, P_A_val_sym, P_B_val_sym),
        ...
        prove_or_find_counterexample(s, f"L9AR Add Assoc (DFI Haven, S={current_local_span_S_py})",
                                     all_L9AR_defs + dfi_haven_conds_L9AR, avc_equiv_local(lhs_L9AR["val_L_sym"], rhs_L9AR["val_L_sym"], P_A_val_sym, P_B_val_sym),
        ...
        prove_or_find_counterexample(s, f"L10AR Distributivity (DFI Haven, S={current_local_span_S_py})",
                                     all_L10AR_defs + dfi_haven_conds_L10AR, avc_equiv_local(lhs_dist_L10AR["val_L_sym"], rhs_dist_L10AR["val_L_sym"], P_A_val_sym, P_B_val_sym),
