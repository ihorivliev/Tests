...
def run_smt_foundational_tests(omega_values_to_test: List[int]):
    ...
    # Current: Add(c,a,id,inv_ex,inv_uq,inv_count_spec), Mul(c,a,zd,fp1_id,fp1_not_id), Distrib, Cancel(a,m) = 14
    # Sub(id,stuck,noncomm,nonassoc, dfi_cases(3parts)) = 4+3 = 7
    # Div(unio(3), dfi(3), round_trip(1)) = 7
    # New Add (qg_exist, pow_assoc, l_alt, r_alt, r_bol) = 5
    # Total approx = 14 + 7 + 7 + 5 = 33 (some are conditional on Omega or sub-parts)
    # The count from your script comment was 35, let's use that as it reflects your latest additions.
