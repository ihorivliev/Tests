...
def avc_div_smt_logic(a_sym: SMTSymbolicAVCVal_Hint, b_sym: SMTSymbolicAVCVal_Hint, 
                      res_sym: SMTSymbolicAVCVal_Hint, omega_smt_node: FNode_Hint) -> Union[FNode_Hint, None]:
    if not SMT_MODE_AVAILABLE: return None
    # ... (ensure inputs are valid dicts, etc. as in your full script) ...
    ...
    q_s = Symbol(f"{res_name_prefix}_q_b3", SMTIntType_RT) 
    r_s = Symbol(f"{res_name_prefix}_r_b3", SMTIntType_RT) 

    euclidean_division_constraints = And(
        Equals(a_sym["val"], Plus(Times(q_s, b_sym["val"]), r_s)), # type: ignore
        r_s >= Int(0), # type: ignore
        # b_sym["val"] will be >= 1 if b_sym is DFI and Omega > 1.
        # If Omega = 1, this DFI/DFI path is not taken due to base constraints on a_sym, b_sym.
        # If Omega > 1, b_sym["val"] > 0 is guaranteed if b_sym["is_finite"] is TRUE.
        r_s < b_sym["val"] # type: ignore 
    )
                                            
    cond_exact_and_valid_dfi_quotient = And(
        Equals(r_s, Int(0)), # type: ignore
        q_s >= Int(1), # type: ignore
        q_s < omega_smt_node # type: ignore
    )
    ...
    # The SMT conditions a_sym["is_finite"] and b_sym["is_finite"] will control
    # whether the DFI/DFI path (rule_b3_consequent_with_euclidean) is relevant.
    # Base constraints on 'a' and 'b' ensure that if Omega=1, is_finite is false.
    div_logic = Ite(b_is_unio, # type: ignore 
                   rule_b1_consequent,
                   Ite(a_is_unio, # type: ignore 
                       rule_b2_consequent,
                       # This final 'else' branch is taken only if both a and b are not Unio,
                       # which means they must be DFI (given their base constraints).
                       # If Omega=1, a_sym["is_finite"] and b_sym["is_finite"] would be False,
                       # so this path (where both are DFI) becomes irrelevant or won't be satisfied
                       # by the solver if the overall context requires a and b to be valid S_Omega elements.
...
def run_smt_foundational_tests(omega_values_to_test: List[int]):
    ...
    # Previous approx 32 + A1(4) + A2(4) from last output calculation based on my suggestion.
    # My previous count of 32 top-level functions. Adding 3 more: AddIdemp, MulIdemp, DFINilpotent
    # So, 32 + 3 = 35 top-level SMT test functions being called.
    # The internal proves in A1 and A2 make the printout longer, but the suite functions are counted.
    # From user's last output, total calls in run_smt_foundational_tests were:
    # Add(8) + Mul(6) + DFI(3) + Mix(1) + Cancel(2) + Sub(5) + Div(3) = 28.
    # Plus the 4+3 = 7 from earlier adds, total 28.
    # Let's recount the functions called in THIS runner:
    # Add: 12; Mul: 6+1=7; DFI: 3+2=5; Mixed: 1; Cancel: 2; Sub: 5+1=6; Div: 3+1=4
    # Total: 12+7+5+1+2+6+4 = 37. Now adding 3 more = 40.
    num_intended_smt_tests_approx = 40
    ...
    for omega_val in omega_values_to_test:
        ...
        smt_test_additive_idempotents(omega_val) # New   
        ...
        smt_test_multiplicative_idempotents(omega_val) # New
        smt_test_existence_of_dfi_multiplicative_nilpotents(omega_val) # New
    ...
    # (This part of your function should remain as it was in your last working script,
    # ensuring it correctly uses 'model_str' and handles the summary counts)
    print("\n--- SMT Test Summary ---")
    total_passed_smt = 0
    total_failed_smt = 0
    total_skipped_smt = 0
        ...
        for failure in sorted(smt_test_failures_details, key=lambda x: x['omega']): # type: ignore
            ...
            if model_is_available and model_str_to_print and model_str_to_print != "N/A" and not isinstance(model_str_to_print, Exception):
                 ...
                 if not ( (is_existential_failure and not observed_holds_failure and "No witness found" in model_str_to_print) or \
                          ...
                          ("SMT Solver Exception" in model_str_to_print and "SMT Solver Exception" in failure['message']) ):
