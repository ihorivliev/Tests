...
def run_smt_foundational_tests(omega_values_to_test: List[int]):
    ...
    # Previous was 32 top-level function calls.
    # New: A1_totality (is 1 func call but 4 internal SMT props), A2_welldefined (1 func call, 4 internal SMT props)
    # Effectively adding 2 top-level function calls, which internally run 4 SMT checks each.
    # For skipped counting, let's count these as 2 * 4 = 8 SMT properties if SMT is unavailable.
    num_intended_smt_tests_approx = 32 + 2 # Count of top-level function calls.
                                          # The prove_or_find counts actual SMT calls.
    ...
    if not SMT_MODE_AVAILABLE:
        ...
        for omega_val in omega_values_to_test:
            ...
            # For the skipped message, it's simpler to count main test suites
            # The internal skipped counts within prove_or_find... will handle per-property skips.
    ...
    for omega_val in omega_values_to_test:
        ...
        # === Foundational Integrity (New explicit tests) ===
