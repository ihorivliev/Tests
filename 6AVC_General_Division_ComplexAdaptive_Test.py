...
if __name__ == "__main__":
    ...
    Fp1_L_val = Symbol("Fp1_L_gdivcadap", solver_INT_TYPE) # For identity test GDIV-CAdap-7,8
    DFI_for_cancel_B = Symbol("DFIB_L_gdivcadap", solver_INT_TYPE) # For GDIV-CAdap-9,10
    ...
    for current_local_span_S_py in OMEGA_VARIANTS_FOR_LOCAL_SPAN_S:
        ...
        # --- GDIV-CAdap-3: ZS_L / A_L ~ Expected (AS_L if A_L not Unio, AS_L if A_L is Unio because ZS_C/Unio_C -> AS_C) ---
        # Simpler: ZS_L / A_L ~ AS_L (since canonical ZS_C / X_C -> ZS_C if X_C is Fp, but X_C/Unio_C -> AS_C)
        # Canonical rule is: ZS_C / Fp_C -> ZS_C. ZS_C / Unio_C -> AS_C.
        s.push()
        ...
        s.add_assertions(a_l_canon_asserts_temp_g3)
        expected_val_gdiv3 = Ite(A_L_canon_temp_g3["is_finite"], P_A_val_sym, P_B_val_sym) # ZS_L if A_L maps to Fp_C, else AS_L
        
        prop_gdiv3 = avc_equiv_local_val(res_gdiv3["val_L_sym"], expected_val_gdiv3, P_A_val_sym, P_B_val_sym)
        prove_or_find_counterexample(s, f"GDIV-CAdap-3 ZS_L / A_L ~ Expected (S={current_local_span_S_py})", res_gdiv3["defining_assertions"], prop_gdiv3,
        ...
        # --- GDIV-CAdap-4: AS_L / A_L ~ AS_L (since canonical AS_C / X_C -> AS_C always) ---
        s.push()
        ...
        s.push()
        # To avoid trivial ZS/ZS, AS/AS cases, let's ensure A and B map to different canonical forms if S allows
        if current_local_span_S_py >= 3: # Omega_eff_C = 7, DFI_C has at least Fp_C(1), Fp_C(2)
             s.add_assertion(Equals(A_L_val, P_A_val_sym + Int(1))) # maps to Fp_C(1)
             s.add_assertion(Equals(B_L_val, P_A_val_sym + Int(2))) # maps to Fp_C(2)
        elif current_local_span_S_py == 2: # Omega_eff_C = 2, DFI_C has Fp_C(1)
             s.add_assertion(Equals(A_L_val, P_A_val_sym + Int(1))) # maps to Fp_C(1)
             s.add_assertion(Equals(B_L_val, P_B_val_sym)) # maps to AS_C
        ...
        if current_local_span_S_py >= 2:
            s.push()
            # A_L_val will be our Fp_L(k)
            s.add_assertion(is_local_DFI_val(A_L_val, P_A_val_sym, P_B_val_sym))
            
            # Expected result is Fp_L(1) = PA_val_sym + 1
            expected_Fp1_val = P_A_val_sym + Int(1)
            # This expectation is only valid if Fp_L(1) itself is a DFI (i.e. S_py >= 2)
            # If S_py = 1, Fp_L(1) is PB, so this test is ill-defined.
            # The map_local_to_complex_adaptive_archetype_input_repr of PA+1 -> Fp_C(1)
            # The result Fp_C(1) maps back to PA+1
            ...
            prop_gdiv8 = Equals(res_gdiv8["val_L_sym"], expected_Fp1_val) # Direct value check, assuming Fp_L(1) maps to itself simply

            # Additional check: is the result actually a DFI that maps to Fp_C(1)?
            res_gdiv8_canon, res_gdiv8_canon_asserts = map_local_to_complex_adaptive_archetype_input_repr(res_gdiv8["val_L_sym"], P_A_val_sym, P_B_val_sym, current_local_span_S_smt, Int(effective_omega_for_print), COMPLEX_CANONICAL_OMEGA_FOR_S_GE_3_PY, "res8_canon")
            s.add_assertions(res_gdiv8_canon_asserts)
            prop_gdiv8_stronger = And(res_gdiv8_canon["is_finite"], Equals(res_gdiv8_canon["val"], Int(1)))


            prove_or_find_counterexample(s, f"GDIV-CAdap-8 Fp_L(k)/Fp_L(k) ~ Fp_L(1) (S={current_local_span_S_py})", 
                                         res_gdiv8["defining_assertions"], prop_gdiv8_stronger, # Check canonical mapping of result
                                         model_vars_to_print=[P_A_val_sym,P_B_val_sym,A_L_val,res_gdiv8["val_L_sym"], expected_Fp1_val, res_gdiv8_canon],
        ...
        if current_local_span_S_py >= 2: # Requires B_L_DFI to exist
            s.push()
            s.add_assertion(is_local_DFI_val(DFI_for_cancel_B, P_A_val_sym, P_B_val_sym)) # B is a DFI
        ...
        if current_local_span_S_py >= 2: # Requires B_L_DFI and result to be DFI
            s.push()
            s.add_assertion(is_local_DFI_val(DFI_for_cancel_B, P_A_val_sym, P_B_val_sym)) # B is a DFI
            ...
            # Constraint: the result of division must be a local DFI
            s.add_assertion(is_local_DFI_val(div_ab_gdiv10["val_L_sym"], P_A_val_sym, P_B_val_sym))
